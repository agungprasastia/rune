#!/usr/bin/env node
// postinstall: fetch the prebuilt `rune` binary for this platform from the
// matching GitHub Release and place it next to bin/rune.js, which execs it.
//
...
//   rune-v{version}-{linux|macos|windows}-{x64|arm64}.{tar.gz|zip}  (+ .sha256)
//
...
//   RUNE_SKIP_DOWNLOAD=1      skip entirely, exit 0 (wrapper will guide if run)
//   RUNE_INSTALL_DRY_RUN=1    print the resolved plan as JSON, no network, exit 0
//   RUNE_INSTALL_PLATFORM=…   override process.platform (linux|darwin|win32|android)
//   RUNE_INSTALL_ARCH=…       override process.arch (x64|arm64)
//   RUNE_REPO=owner/name      override the GitHub repo (default rune-ai/rune)
//   RUNE_GITHUB_BASE_URL=…    override the download host (default https://github.com)
...
const REPO = process.env.RUNE_REPO || 'rune-ai/rune';
const BASE = (process.env.RUNE_GITHUB_BASE_URL || 'https://github.com').replace(/\/+$/, '');
...
const ALLOW_INSECURE = process.env.RUNE_ALLOW_INSECURE_DOWNLOAD === '1';
const MAX_DOWNLOAD_BYTES = 512 * 1024 * 1024;

function fail(message) {
  console.error(`[rune] ${message}`);
  process.exit(1);
}
...
if (process.env.RUNE_SKIP_DOWNLOAD === '1') {
  warnSkip('RUNE_SKIP_DOWNLOAD=1 set — skipping native binary download.');
}

const rawPlatform = process.env.RUNE_INSTALL_PLATFORM || process.platform;
const rawArch = process.env.RUNE_INSTALL_ARCH || process.arch;
...
if (!platform || !arch) {
  warnSkip(
    `no prebuilt binary for ${rawPlatform}/${rawArch}. Build from source: ` +
       `https://github.com/${REPO} (go run ./cmd/rune).`,
  );
}
...
const binaryName = platform === 'windows' ? 'rune.exe' : 'rune';
const tag = `v${VERSION}`;
const assetName = `rune-v${VERSION}-${platform}-${arch}.${ext}`;
...
const optionalBinaries =
  platform === 'windows'
     ? ['rune-windows-command-runner.exe', 'rune-windows-sandbox-setup.exe']
    : platform === 'linux'
      ? ['zero-linux-sandbox', 'zero-seccomp']
      : [];

if (process.env.RUNE_INSTALL_DRY_RUN === '1') {
  ...
  if (!ALLOW_INSECURE && !url.startsWith('https://')) {
    fail(
      `refusing insecure download origin for ${url}: only https:// is allowed ` +
        `(set RUNE_ALLOW_INSECURE_DOWNLOAD=1 to override for local testing).`,
    );
  }
  let response;
  try {
  ...
  if (!response.ok) {
    fail(
      `download failed (HTTP ${response.status}) for ${url} (release tag ${tag}). ` +
        ...
        `build from source: https://github.com/${REPO} (go run ./cmd/rune).`,
    );
  }
...
async function main() {
  ...
  try {
    ...
    for (const name of optionalBinaries) {
      ...
      if (!source) {
        console.error(
          `[rune] note: optional helper ${name} was not in ${assetName}; ` +
    ...
    console.error(`[rune] installed ${binaryName} ${VERSION} (${platform}-${arch}, ${sizeKb} KB).`);
  } finally {
