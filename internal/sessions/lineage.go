...
func (store *Store) ListChildren(parentSessionID string) ([]Metadata, error) {
	if !ValidSessionID(parentSessionID) {
		return nil, fmt.Errorf("invalid rune session id %q", parentSessionID)
	}
	parent, err := store.Get(parentSessionID)
	if err != nil {
		return nil, err
	...
	if parent == nil {
		return nil, fmt.Errorf("rune session not found: %s", parentSessionID)
	}
	all, err := store.List()
	if err != nil {
		return nil, err
...
func (store *Store) Lineage(sessionID string) ([]Metadata, error) {
	...
	for currentID != "" {
		if seen[currentID] {
			return nil, fmt.Errorf("cycle in rune session lineage at %s", currentID)
		}
		seen[currentID] = true
		...
		if session == nil {
			return nil, fmt.Errorf("rune session not found: %s", currentID)
		}
...
func (store *Store) Tree(rootSessionID string) (TreeNode, error) {
	if !ValidSessionID(rootSessionID) {
		return TreeNode{}, fmt.Errorf("invalid rune session id %q", rootSessionID)
	}
	...
	if root == nil {
		...
		return TreeNode{}, fmt.Errorf("rune session not found: %s", rootSessionID)
	}
...
func (store *Store) treeFrom(sessionID string, byID map[string]Metadata, childrenByParent map[string][]Metadata, seen map[string]bool) (TreeNode, error) {
	if seen[sessionID] {
		return TreeNode{}, fmt.Errorf("cycle in rune session tree at %s", sessionID)
	}
	seen[sessionID] = true
	session, ok := byID[sessionID]
	if !ok {
		return TreeNode{}, fmt.Errorf("rune session not found: %s", sessionID)
	}
