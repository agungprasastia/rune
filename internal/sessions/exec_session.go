...
func PrepareExec(options PrepareExecOptions) (PreparedExec, error) {
	...
	if forkID != "" {
		...
		if parent == nil {
			return PreparedExec{}, ExecError{"Rune session not found: " + forkID}
		}
	...
	if resumeID != "" || options.ResumeLatest {
		...
		if sessionID == "" && options.ResumeLatest {
			...
			if latest == nil {
				return PreparedExec{}, ExecError{"No Rune sessions available to resume."}
			}
		...
		if session == nil {
			return PreparedExec{}, ExecError{"Rune session not found: " + sessionID}
		}
		if !IsResumableKind(session.SessionKind) {
			return PreparedExec{}, ExecError{"Rune session is not resumable: " + sessionID}
		}
	...
	if strings.TrimSpace(options.CallingSessionID) != "" {
		...
		if parent == nil {
			return PreparedExec{}, ExecError{"Rune parent session not found: " + parentSessionID}
		}
...
func readExecContextEvents(store *Store, sessionID string) ([]Event, error) {
	...
	log.Printf("rune sessions: failed to rehydrate compaction events for %s; falling back to raw events: %v", sessionID, err)
...
func FormatExecPrompt(prompt string, prepared PreparedExec) string {
	...
	return strings.Join([]string{
		fmt.Sprintf("%s Rune session %s.", label, sessionID),
