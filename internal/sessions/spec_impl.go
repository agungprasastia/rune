...
func (store *Store) FindSpecImplementation(specID string, sourceSessionID string) (*Metadata, error) {
	...
	if !ValidSessionID(sourceSessionID) {
		return nil, fmt.Errorf("invalid rune session id %q", sourceSessionID)
	}
...
func (store *Store) EnsureSpecImplementation(input EnsureSpecImplementationInput) (Metadata, []Event, error) {
	...
	prompt := input.Prompt
	if specID == "" {
		return Metadata{}, nil, fmt.Errorf("rune spec id is required")
	}
	if sourceSessionID == "" {
		return Metadata{}, nil, fmt.Errorf("rune spec source session id is required")
	}
	if !ValidSessionID(sourceSessionID) {
		return Metadata{}, nil, fmt.Errorf("invalid rune session id %q", sourceSessionID)
	}
	if strings.TrimSpace(prompt) == "" {
		return Metadata{}, nil, fmt.Errorf("rune spec implementation prompt is required")
	}
	...
	if source == nil {
		return Metadata{}, nil, fmt.Errorf("rune session not found: %s", sourceSessionID)
	}
	...
	if loaded == nil {
		return Metadata{}, nil, fmt.Errorf("rune session not found: %s", impl.SessionID)
	}
