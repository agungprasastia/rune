...
func DefaultRoot(env map[string]string) string {
	...
	return filepath.Join(base, "rune", "sessions")
}
...
func (store *Store) Create(input CreateInput) (Metadata, error) {
	...
	if !ValidSessionID(sessionID) {
		return Metadata{}, fmt.Errorf("invalid rune session id %q", input.SessionID)
	}
	if input.Depth < 0 {
		return Metadata{}, fmt.Errorf("invalid rune session depth %d", input.Depth)
	}
	...
	if err := os.MkdirAll(store.RootDir, 0o700); err != nil {
		return Metadata{}, fmt.Errorf("create rune session root: %w", err)
	}
	if err := os.Mkdir(store.sessionPath(sessionID), 0o700); err != nil {
		if errors.Is(err, os.ErrExist) {
			return Metadata{}, fmt.Errorf("rune session already exists: %s", sessionID)
		}
		return Metadata{}, fmt.Errorf("create rune session directory: %w", err)
	}
	...
	if err != nil {
		return Metadata{}, fmt.Errorf("create rune session events file: %w", err)
	}
	if err := file.Close(); err != nil {
		return Metadata{}, fmt.Errorf("close rune session events file: %w", err)
	}
	return session, nil
}
...
func (store *Store) Get(sessionID string) (*Metadata, error) {
	if !ValidSessionID(sessionID) {
		return nil, fmt.Errorf("invalid rune session id %q", sessionID)
	}
...
func (store *Store) List() ([]Metadata, error) {
	if err := os.MkdirAll(store.RootDir, 0o700); err != nil {
		return nil, fmt.Errorf("create rune session root: %w", err)
	}
	entries, err := os.ReadDir(store.RootDir)
	if err != nil {
		return nil, fmt.Errorf("read rune session root: %w", err)
	}
	sessions := []Metadata{}
...
func (store *Store) Fork(parentSessionID string, input ForkInput) (Metadata, error) {
	if !ValidSessionID(parentSessionID) {
		return Metadata{}, fmt.Errorf("invalid rune session id %q", parentSessionID)
	}
	...
	if parent == nil {
		return Metadata{}, fmt.Errorf("rune session not found: %s", parentSessionID)
	}
	...
	if kind != SessionKindFork && kind != SessionKindSide {
		return Metadata{}, fmt.Errorf("invalid rune fork session kind %q", kind)
	}
...
func (store *Store) RecordSpec(sessionID string, input RecordSpecInput) (Metadata, Event, error) {
	...
	if status == "" {
		return Metadata{}, Event{}, fmt.Errorf("rune spec status is required")
	}
...
func (store *Store) AppendEventUnlessExists(sessionID string, input AppendEventInput, exists func([]Event) bool) (Event, bool, error) {
	if !ValidSessionID(sessionID) {
		return Event{}, false, fmt.Errorf("invalid rune session id %q", sessionID)
	}
	if strings.TrimSpace(string(input.Type)) == "" {
		return Event{}, false, fmt.Errorf("rune session event type is required")
	}
	unlock, err := store.lockSession(sessionID)
	if err != nil {
...
func (store *Store) appendPreparedEventsLocked(sessionID string, inputs []preparedAppendEvent) ([]Event, error) {
	...
	for index, input := range inputs {
		...
		if err != nil {
			return nil, fmt.Errorf("encode rune session event: %w", err)
		}
		batch.Write(data)
		batch.WriteByte('\n')
	...
	if err != nil {
		return nil, fmt.Errorf("append rune session event: %w", err)
	}
	...
	if err := file.Sync(); err != nil {
		_ = file.Close()
		return nil, fmt.Errorf("sync rune session event: %w", err)
	}
	if err := file.Close(); err != nil {
		return nil, fmt.Errorf("close rune session event file: %w", err)
	}
	last := events[len(events)-1]
...
func (store *Store) UpdateTitle(sessionID string, title string) (Metadata, error) {
	...
	if trimmed == "" {
		return Metadata{}, fmt.Errorf("rune session title is required")
	}
	unlock, err := store.lockSession(sessionID)
	if err != nil {
		return Metadata{}, err
...
func (store *Store) UpdateTitleIfCurrent(sessionID string, expected string, title string) (Metadata, bool, error) {
	if !ValidSessionID(sessionID) {
		return Metadata{}, false, fmt.Errorf("invalid rune session id %q", sessionID)
	}
	trimmed := strings.TrimSpace(title)
	if trimmed == "" {
		return Metadata{}, false, fmt.Errorf("rune session title is required")
	}
	unlock, err := store.lockSession(sessionID)
	if err != nil {
...
func (store *Store) ReadEvents(sessionID string) ([]Event, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("read rune session events: %w", err)
	}
	...
	for index, line := range lines {
		...
		if err := json.Unmarshal(line, &event); err != nil {
			...
			return nil, fmt.Errorf("invalid json in rune session %s %s at line %d: %w", sessionID, EventsFile, index+1, err)
		}
		events = append(events, event)
	}
	return events, nil
...
func (store *Store) readMetadata(sessionID string) (Metadata, error) {
	...
	if err := json.Unmarshal(data, &session); err != nil {
		return Metadata{}, fmt.Errorf("invalid rune session metadata %s: %w", sessionID, err)
	}
	return session, nil
}
...
func (store *Store) writeMetadata(session Metadata) error {
	...
	if err != nil {
		return fmt.Errorf("encode rune session metadata: %w", err)
	}
	...
	if err := writeFileSync(tmp, append(data, '\n'), 0o600); err != nil {
		return fmt.Errorf("write rune session metadata: %w", err)
	}
	if err := fsutil.RenameWithRetry(tmp, path, nil); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("replace rune session metadata: %w", err)
	}
	...
	if err := syncDir(filepath.Dir(path)); err != nil {
		return fmt.Errorf("sync rune session dir: %w", err)
	}
	return nil
}
...
func rawPayload(payload any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode rune session payload: %w", err)
	}
	return data, nil
}
...
func prepareAppendEventInputs(inputs []AppendEventInput) ([]preparedAppendEvent, error) {
	...
	for _, input := range inputs {
		if strings.TrimSpace(string(input.Type)) == "" {
			return nil, fmt.Errorf("rune session event type is required")
		}
