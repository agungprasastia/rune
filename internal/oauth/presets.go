...
// (as if RUNE_OAUTH_ALLOW_PRESETS=1 were exported) so an interactive wizard/CLI
...
// drop the operator's RUNE_OAUTH_<NAME>_* overrides. The flag is then forced on.
...
// provider. Every field is overridable per provider via RUNE_OAUTH_<NAME>_*
...
// with RUNE_OAUTH_ALLOW_PRESETS (see presetsAllowed). A preset carries a
...
// RUNE_OAUTH_<NAME>_* override is set.
//
...
// change without notice (override via RUNE_OAUTH_XAI_*), and using it requires a
...
var builtinOAuthPresets = map[string]providerPreset{
	...
	// pre-filled; the operator supplies RUNE_OAUTH_HUGGINGFACE_CLIENT_ID from
...
// are OFF unless the operator opts in with RUNE_OAUTH_ALLOW_PRESETS set to a
...
func presetsAllowed(env map[string]string) bool {
	switch strings.ToLower(strings.TrimSpace(envValue(env, "RUNE_OAUTH_ALLOW_PRESETS"))) {
	case "1", "true", "yes", "on":
		return true
	}
	return false
