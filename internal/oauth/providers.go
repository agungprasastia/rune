...
// is defined entirely by the operator via RUNE_OAUTH_<NAME>_* variables, so no
...
// RUNE_OAUTH_ALLOW_PRESETS (see presetsAllowed); env always overrides a preset.
type Registry struct{}

// NewRegistry returns the (stateless) env-driven registry.
func NewRegistry() *Registry { return &Registry{} }

// envKey builds the RUNE_OAUTH_<NAME>_<suffix> variable name for a provider.
func envKey(name, suffix string) string {
	up := strings.ToUpper(name)
	up = strings.NewReplacer("-", "_", ".", "_").Replace(up)
	return "RUNE_OAUTH_" + up + "_" + suffix
}
...
//	RUNE_OAUTH_<NAME>_AUTHORIZE_URL   (loopback flow; or discovered via issuer)
//	RUNE_OAUTH_<NAME>_TOKEN_URL       (or discovered via issuer)
//	RUNE_OAUTH_<NAME>_DEVICE_URL      (device flow; or discovered via issuer)
//	RUNE_OAUTH_<NAME>_ISSUER_URL      (RFC 8414 / OIDC discovery base)
//	RUNE_OAUTH_<NAME>_SCOPES          (space-separated)
//	RUNE_OAUTH_<NAME>_FLOW            ("loopback" [default] | "device")
//
...
func (r *Registry) ResolveConfig(name string, env map[string]string) (Config, Flow, error) {
	...
	// in with RUNE_OAUTH_ALLOW_PRESETS — otherwise no third-party client identity is
	// used and every field must come from a RUNE_OAUTH_<NAME>_* env var (env wins).
	...
	if cfg.ClientID == "" {
		...
		if _, ok := lookupOAuthPreset(name); ok {
			hint = " (or set RUNE_OAUTH_ALLOW_PRESETS=1 to use the built-in preset)"
		}
