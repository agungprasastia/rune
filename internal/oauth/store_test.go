...
func TestEnvValueHermetic(t *testing.T) {
	t.Setenv("RUNE_OAUTH_DEMO_CLIENT_ID", "ambient")
	// A non-nil map omitting the key must NOT leak the ambient process value.
	if got := envValue(map[string]string{}, "RUNE_OAUTH_DEMO_CLIENT_ID"); got != "" {
		t.Fatalf("non-nil env map must be hermetic, got %q", got)
	}
	// A nil map reads the process environment.
	if got := envValue(nil, "RUNE_OAUTH_DEMO_CLIENT_ID"); got != "ambient" {
...
func TestResolveStorePathHonorsOverride(t *testing.T) {
	...
	path, err := ResolveStorePath(map[string]string{"RUNE_OAUTH_TOKENS_PATH": override})
	if err != nil {
...
// through ProviderKey, so `rune auth login xAI` and the scaffolded profile's
