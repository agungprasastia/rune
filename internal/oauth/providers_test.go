...
func TestResolveConfigFromEnv(t *testing.T) {
	...
	env := map[string]string{
		"RUNE_OAUTH_DEMO_CLIENT_ID":     "my-client",
		"RUNE_OAUTH_DEMO_CLIENT_SECRET": "shh",
		"RUNE_OAUTH_DEMO_SCOPES":        "read write",
		"RUNE_OAUTH_DEMO_AUTHORIZE_URL": "https://auth.example.com/authorize",
		"RUNE_OAUTH_DEMO_TOKEN_URL":     "https://auth.example.com/token",
	}
...
func TestResolveConfigDeviceFlow(t *testing.T) {
	r := NewRegistry()
	env := map[string]string{
		"RUNE_OAUTH_DEMO_CLIENT_ID":  "c",
		"RUNE_OAUTH_DEMO_TOKEN_URL":  "https://auth.example.com/token",
		"RUNE_OAUTH_DEMO_DEVICE_URL": "https://auth.example.com/device",
		"RUNE_OAUTH_DEMO_FLOW":       "device",
	}
...
func TestResolveConfigRequiresEndpointsOrIssuer(t *testing.T) {
	...
	_, _, err := r.ResolveConfig("demo", map[string]string{"RUNE_OAUTH_DEMO_CLIENT_ID": "c"})
	if err == nil {
...
func TestResolveConfigRejectsInsecureEndpoint(t *testing.T) {
	...
	env := map[string]string{
		"RUNE_OAUTH_DEMO_CLIENT_ID":     "c",
		"RUNE_OAUTH_DEMO_AUTHORIZE_URL": "https://auth.example.com/authorize",
		"RUNE_OAUTH_DEMO_TOKEN_URL":     "http://insecure.example/token", // non-https, non-loopback
	}
...
func TestEnvKey(t *testing.T) {
	if got := envKey("my-svc", "CLIENT_ID"); got != "RUNE_OAUTH_MY_SVC_CLIENT_ID" {
		t.Fatalf("envKey = %q", got)
	}
	if got := envKey("two.part", "SCOPES"); got != "RUNE_OAUTH_TWO_PART_SCOPES" {
		t.Fatalf("envKey = %q", got)
	}
}
