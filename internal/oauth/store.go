...
// `rune auth login xAI` stored "provider:xAI" while the profile scaffolded for
...
type StoreOptions struct {
	...
	// keyring. When empty it falls back to RUNE_OAUTH_STORAGE.
	Storage string
...
// honoring RUNE_OAUTH_TOKENS_PATH, then XDG_CONFIG_HOME, then the home dir.
func ResolveStorePath(env map[string]string) (string, error) {
	if override := strings.TrimSpace(envValue(env, "RUNE_OAUTH_TOKENS_PATH")); override != "" {
	...
	return filepath.Join(configHome, "rune", "oauth-tokens.json"), nil
}

// NewStore builds a token store with the configured backend (file by default,
// or the OS keyring when Storage/RUNE_OAUTH_STORAGE selects it).
func NewStore(options StoreOptions) (*Store, error) {
	...
	if storage == "" {
		storage = strings.TrimSpace(envValue(options.Env, "RUNE_OAUTH_STORAGE"))
	}
...
// RUNE_OAUTH_* / HOME / XDG_CONFIG_HOME values. Only a nil map uses os.Getenv.
