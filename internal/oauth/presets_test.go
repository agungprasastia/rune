...
func TestResolveConfigUsesXAIPreset(t *testing.T) {
	r := NewRegistry()
	// Presets are opt-in; with RUNE_OAUTH_ALLOW_PRESETS set and no other env vars
	// the preset supplies everything.
	cfg, flow, err := r.ResolveConfig("xai", map[string]string{"RUNE_OAUTH_ALLOW_PRESETS": "1"})
	if err != nil {
...
// opt into the preset without exporting RUNE_OAUTH_ALLOW_PRESETS: an otherwise-inert
...
// The helper copies the base map (never mutating it) and keeps RUNE_OAUTH_<NAME>_*
...
func TestEnvWithPresetsAllowedPreservesOverrides(t *testing.T) {
	base := map[string]string{"RUNE_OAUTH_XAI_CLIENT_ID": "custom-id"}
	env := envWithPresetsAllowed(base)
	if env["RUNE_OAUTH_ALLOW_PRESETS"] != "1" {
		t.Fatalf("opt-in flag not set: %v", env)
	}
	if _, mutated := base["RUNE_OAUTH_ALLOW_PRESETS"]; mutated {
...
func TestResolveConfigEnvOverridesPreset(t *testing.T) {
	...
	env := map[string]string{
		"RUNE_OAUTH_ALLOW_PRESETS": "1",
		"RUNE_OAUTH_XAI_CLIENT_ID": "custom-id",
		"RUNE_OAUTH_XAI_SCOPES":    "alpha beta",
		"RUNE_OAUTH_XAI_FLOW":      "device",
	}
...
func TestResolveConfigPresetInertWithoutOptIn(t *testing.T) {
	...
	if err == nil {
		t.Fatal("xai must not resolve from the preset unless RUNE_OAUTH_ALLOW_PRESETS is set")
	}
	if !strings.Contains(err.Error(), "RUNE_OAUTH_ALLOW_PRESETS") {
...
func TestResolveConfigHuggingFaceRequiresClientID(t *testing.T) {
	r := NewRegistry()
	_, _, err := r.ResolveConfig("huggingface", map[string]string{"RUNE_OAUTH_ALLOW_PRESETS": "1"})
	if err == nil {
		t.Fatal("huggingface must not resolve from the preset alone (no baked-in client_id)")
	}
	if !strings.Contains(err.Error(), "RUNE_OAUTH_HUGGINGFACE_CLIENT_ID") {
...
func TestResolveConfigHuggingFaceWithEnvClientID(t *testing.T) {
	...
	env := map[string]string{
		"RUNE_OAUTH_ALLOW_PRESETS":         "1",
		"RUNE_OAUTH_HUGGINGFACE_CLIENT_ID": "test-client-id-value",
		"RUNE_OAUTH_HUGGINGFACE_SCOPES":    "openid inference-api",
	}
...
func TestResolveConfigChatGPTPreset(t *testing.T) {
	r := NewRegistry()
	cfg, flow, err := r.ResolveConfig("chatgpt", map[string]string{"RUNE_OAUTH_ALLOW_PRESETS": "1"})
	if err != nil {
