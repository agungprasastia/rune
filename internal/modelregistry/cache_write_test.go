...
func TestCalculateCostCacheWritePremium(t *testing.T) {
	...
	usage := runeruntime.Usage{InputTokens: 1_000_000, CachedInputTokens: 300_000, CacheWriteTokens: 100_000, OutputTokens: 200_000}
...
func TestCalculateCostCacheWriteFallsBackToInputRate(t *testing.T) {
	...
	usage := runeruntime.Usage{InputTokens: 1_000_000, CacheWriteTokens: 100_000, OutputTokens: 0}
	cost, err := CalculateCost(model, usage)
	if err != nil {
		t.Fatal(err)
	}
