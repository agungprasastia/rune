...
func (registry Registry) Require(pattern string) (ModelEntry, error) {
	model, ok := registry.Get(pattern)
	if !ok {
		return ModelEntry{}, fmt.Errorf("unknown Rune model %q", pattern)
	}
	return model, nil
}
...
func (registry Registry) RequireProvider(pattern string, provider ProviderKind) (ModelEntry, error) {
	...
	if model.Provider != provider {
		return ModelEntry{}, fmt.Errorf("rune model %s belongs to %s, not %s", model.ID, model.Provider, provider)
	}
	return model, nil
}
