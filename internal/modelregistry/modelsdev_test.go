...
func TestApplyModelsDevOverrides(t *testing.T) {
	...
	t.Setenv("RUNE_MODELS_CACHE_PATH", filepath.Join(t.TempDir(), "absent.json"))
...
func TestRefreshModelsDevCacheFetchesAndCaches(t *testing.T) {
	...
	t.Setenv("RUNE_MODELS_CACHE_PATH", cachePath)
	t.Setenv("RUNE_MODELS_URL", server.URL)
	t.Setenv("RUNE_DISABLE_MODELS_FETCH", "")
...
func TestRefreshModelsDevCacheDisabledByEnv(t *testing.T) {
	...
	defer server.Close()
	t.Setenv("RUNE_MODELS_CACHE_PATH", filepath.Join(t.TempDir(), "modelsdev.json"))
	t.Setenv("RUNE_MODELS_URL", server.URL)
	t.Setenv("RUNE_DISABLE_MODELS_FETCH", "1")
	if err := RefreshModelsDevCache(t.Context()); err != nil {
		t.Fatal(err)
	}
}
