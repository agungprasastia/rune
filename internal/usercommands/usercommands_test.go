...
func TestLoadParsesFrontmatterAndBody(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, ".rune", "commands")
	...
	if !c.Project {
		t.Fatal("expected Project=true for a .rune/commands file")
	}
}
...
func TestLoadProjectOverridesUser(t *testing.T) {
	root := t.TempDir()
	userCfg := t.TempDir()
	writeFile(t, filepath.Join(userCfg, "rune", "commands"), "review.md", "USER version")
	writeFile(t, filepath.Join(root, ".rune", "commands"), "review.md", "PROJECT version")
