...
func Format(report Report) string {
	lines := []string{
		fmt.Sprintf("Rune doctor report (%s)", redaction.RedactString(report.GeneratedAt, redaction.Options{})),
...
func runtimeCheck(runtime string) Check {
	...
	return check("runtime.go", "Go runtime", StatusPass, fmt.Sprintf("Rune Go runtime is available (%s).", runtime), map[string]any{"runtime": runtime})
}
...
func configFilesCheck(userPath string, projectPath string) Check {
	...
	if len(details) == 0 {
		return check("config.files", "Config files", StatusWarn, "No explicit Rune config files were inspected.", nil)
	}
	return check("config.files", "Config files", StatusPass, "Rune config file inputs are available for inspection.", details)
}
...
func providerConfigCheck(profile config.ProviderProfile) Check {
	...
	// inline fields made doctor and `rune providers list` disagree about the
	...
	if credential == "not set" && !localProviderBaseURL(profile.BaseURL) {
		return check("provider.config", "Provider config", StatusFail,
			fmt.Sprintf("No API key configured for %s. Run `rune auth` or `rune setup`, or set the provider's API key environment variable.", providerName(profile)),
			details)
	}
...
func providerModelCheck(profile config.ProviderProfile) Check {
	...
	if err != nil {
		if profile.ProviderKind == config.ProviderKindOpenAICompatible || profile.ProviderKind == config.ProviderKindAnthropicCompat {
			return check("provider.model", "Provider model", StatusWarn, fmt.Sprintf("Custom %s model was not found in the Rune registry; runtime will pass it through to the configured provider. Run `rune doctor --connectivity` to validate the endpoint and auth.", profile.ProviderKind), map[string]any{"model": profile.Model, "provider": providerName(profile)})
		}
...
func connectivityCheck(profile config.ProviderProfile, enabled bool, modelStatus Status, health *providerhealth.Result) Check {
	if !enabled {
		...
		return check("provider.connectivity", "Provider connectivity", StatusWarn, "Connectivity probe skipped. Run `rune doctor --connectivity` to probe the provider endpoint.", map[string]any{"baseURL": profile.BaseURL})
	}
	if health != nil {
...
func configValidationCheck(userPath string, projectPath string) Check {
	...
	if len(paths) == 0 {
		return check("config.validation", "Config validation", StatusWarn, "No Rune config files were available to validate.", nil)
	}
	...
	if status == StatusPass {
		return check("config.validation", "Config validation", StatusPass, "Rune config files parsed and validated successfully.", nil)
	}
	return check("config.validation", "Config validation", StatusFail, fmt.Sprintf("Rune config validation found %d issue(s).", issueCount), details)
}
