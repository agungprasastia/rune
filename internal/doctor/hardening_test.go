...
func TestSandboxCheckReportsWindowsNativeSetupStates(t *testing.T) {
	...
	t.Setenv("RUNE_WINDOWS_SANDBOX_HOME", t.TempDir())
	...
	if setupStatus != "missing-or-out-of-date" || !strings.Contains(setupRemedy, "rune sandbox setup") {
	...
	// on PATH for the Windows backend to be usable — the running rune binary acts
	...
	// require a setup marker) the check passes. The actionable "run `rune sandbox
