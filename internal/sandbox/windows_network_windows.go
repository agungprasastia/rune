...
func openWindowsWFPEngine() (windows.Handle, error) {
	sessionName, err := windows.UTF16PtrFromString("Rune Windows Sandbox WFP")
	if err != nil {
		return 0, err
	}
...
func ensureWindowsWFPProvider(engine windows.Handle, providerKey windows.GUID) error {
	name, err := windows.UTF16PtrFromString("Rune Windows Sandbox WFP")
	if err != nil {
		return err
	}
	description, err := windows.UTF16PtrFromString("Persistent WFP provider for Rune Windows sandbox filters")
	if err != nil {
		return err
	}
...
func ensureWindowsWFPSubLayer(engine windows.Handle, providerKey windows.GUID, subLayerKey windows.GUID) error {
	...
	description, err := windows.UTF16PtrFromString("Persistent WFP sublayer for Rune Windows sandbox filters")
	if err != nil {
		return err
	}
