...
const WindowsSandboxCommandRunnerName = "rune-windows-command-runner.exe"

const windowsCapabilitySIDSchemaVersion = 2

// Hidden subcommands the main rune binary answers to when it acts as its own
...
// self-dispatch — the running rune binary acting as its own helper — Name is
...
// wins; (2) the same name on PATH; (3) SELF-DISPATCH — the running rune binary
...
func windowsRestrictedTokenCommandPlan(execRequest SandboxExecutionRequest, policy Policy) (CommandPlan, error) {
	...
	// subcommand token (Name is the running rune binary), so the launched command
	// is `rune __windows-command-runner <sandbox args>`. Empty for a standalone
...
func ResolveWindowsSandboxHome(env map[string]string) (string, error) {
	if override := strings.TrimSpace(envValue(env, "RUNE_WINDOWS_SANDBOX_HOME")); override != "" {
