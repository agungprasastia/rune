...
func TestIsAlreadySandboxed(t *testing.T) {
	// A lone RUNE_SANDBOXED=1 (no corroborating backend marker) must NOT count as
	...
	// The backend marker alone (no RUNE_SANDBOXED=1) must not count either.
...
func TestBuildCommandPlanWrapsWhenNotAlreadySandboxed(t *testing.T) {
	...
	if !plan.Wrapped || plan.Name != "/usr/bin/rune-linux-sandbox" {
