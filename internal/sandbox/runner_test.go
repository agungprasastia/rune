...
func TestBuildCommandPlanWrapsLinuxHelper(t *testing.T) {
	...
	if !plan.Wrapped || plan.Name != "/usr/bin/rune-linux-sandbox" || plan.Backend.Name != BackendLinuxBwrap {
...
func TestSandboxExecProfileAllowsDevNullAndTemp(t *testing.T) {
	...
	if out, err := run("echo leak > /etc/rune_sandbox_should_fail 2>/dev/null"); err == nil {
...
func TestLinuxHelperPlanCarriesExtraWriteRoots(t *testing.T) {
	...
	engine := NewEngine(EngineOptions{
		...
		Policy:        DefaultPolicy(),
		Scope:         scope,
		Backend:       Backend{Name: BackendLinuxBwrap, Available: true, Executable: "/usr/bin/rune-linux-sandbox"},
	})
...
func TestLinuxHelperPlanPreservesRealExtraRootCwd(t *testing.T) {
	...
	bwrapArgs, err := BuildLinuxSandboxBwrapArgs(LinuxSandboxBwrapOptions{Config: config, HelperPath: "/usr/bin/rune-linux-sandbox"})
	if err != nil {
