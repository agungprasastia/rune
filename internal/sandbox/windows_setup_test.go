...
func TestWindowsSandboxSetupPathForRunner(t *testing.T) {
	got := WindowsSandboxSetupPathForRunner(filepath.Join("C:", "rune", WindowsSandboxCommandRunnerName))
	want := filepath.Join("C:", "rune", WindowsSandboxSetupName)
	if got != want {
...
func TestWindowsSandboxSetupConfigFromCommandPreservesProfileInputs(t *testing.T) {
	command := WindowsSandboxCommandConfig{
		...
		Env:     map[string]string{"RUNE_SANDBOXED": "1"},
