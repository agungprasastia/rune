...
// EnvSandboxed marks a process that rune has already wrapped in a sandbox: every
// wrapped command carries RUNE_SANDBOXED=1 in its environment. When such a
...
// default.
const EnvSandboxed = "RUNE_SANDBOXED"
...
// ambient flag (a stray or hand-exported RUNE_SANDBOXED=1 with no backend marker
// no longer forces an unsandboxed pass-through).
const EnvSandboxBackend = "RUNE_SANDBOX_BACKEND"

// IsAlreadySandboxed reports whether the current process is already running
// inside a rune-created sandbox. It requires BOTH correlated markers that
// sandboxEnvironment sets together — EnvSandboxed == "1" AND a non-empty
// EnvSandboxBackend — so a single user-set/inherited RUNE_SANDBOXED=1 cannot by
// itself disable wrapping. rune sets both only on genuinely wrapped commands;
...
const (
	...
	// elevated `rune sandbox setup` has not run: commands still wrap through the
