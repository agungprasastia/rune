...
// REQUIRED the sandbox (--sandbox require) on Windows before `rune sandbox setup`
...
var errWindowsSandboxNotInitialized = errors.New(
	"the Windows sandbox is not initialized — run `rune sandbox setup` from an elevated (Administrator) terminal, " +
...
const windowsSetupDowngradeReason = "Windows OS sandbox inactive — commands run with workspace path-confinement and " +
	"per-command approval only. Run `rune sandbox setup` from an elevated (Administrator) terminal for full filesystem and network isolation."
...
const windowsUnelevatedDowngradeReason = "Windows sandbox running unelevated — the filesystem write-jail is enforced, but network " +
	"isolation still relies on per-command approval. Run `rune sandbox setup` from an elevated (Administrator) terminal for OS-level network enforcement."
...
func (manager SandboxManager) BuildExecutionRequest(request SandboxManagerRequest) (SandboxExecutionRequest, error) {
	...
	// Windows: the FULL OS sandbox needs a one-time elevated `rune sandbox setup`
