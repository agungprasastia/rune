...
func runWindowsSandboxSetup(config WindowsSandboxSetupConfig, stderr io.Writer) int {
	...
	if !windowsProcessIsElevated() {
		fmt.Fprintln(stderr, WindowsSandboxSetupName+": Administrator rights are required. Re-run `rune sandbox setup` from an elevated (Run as administrator) terminal.")
		return 1
	}
