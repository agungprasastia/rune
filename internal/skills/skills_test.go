...
func TestDefaultDirHonorsEnvOverride(t *testing.T) {
	got := DefaultDir(map[string]string{"RUNE_SKILLS_DIR": "/custom/skills"})
...
func TestDefaultDirHonorsXDGDataHome(t *testing.T) {
	...
	want := filepath.Join("/xdg/data", "rune", "skills")
	if got != want {
...
func TestDefaultDirFallsBackToHome(t *testing.T) {
	got := DefaultDir(map[string]string{"HOME": "/home/rune"})
	want := filepath.Join("/home/rune", ".local", "share", "rune", "skills")
	if got != want {
		t.Fatalf("DefaultDir = %q, want %q", got, want)
	}
}
...
func TestAgentsDirIgnoresZeroSkillsDir(t *testing.T) {
	...
	// RUNE_SKILLS_DIR must not redirect or suppress AgentsDir.
	got := AgentsDir(map[string]string{
		"HOME":            home,
		"RUNE_SKILLS_DIR": filepath.Join(home, "rune-only"),
	})
	if got != agents {
		t.Fatalf("AgentsDir with RUNE_SKILLS_DIR set = %q, want %q", got, agents)
	}
}
...
func TestAgentsDirUnresolvableHomeIsEmpty(t *testing.T) {
	...
	// and that a deliberately empty-looking override path is not invented from RUNE.
...
func TestDiscoveryRootsOrderAndOmission(t *testing.T) {
	...
	primary := filepath.Join(home, "rune-skills")
	env := map[string]string{
		"HOME":            home,
		"RUNE_SKILLS_DIR": primary,
	}
