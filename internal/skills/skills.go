...
// explicit RUNE_SKILLS_DIR override wins; otherwise it is
// $XDG_DATA_HOME/rune/skills or ~/.local/share/rune/skills. The directory is
...
func DefaultDir(env map[string]string) string {
	if override := strings.TrimSpace(envValue(env, "RUNE_SKILLS_DIR")); override != "" {
		return override
	}
	...
	if base == "" {
		if home == "" {
			...
			// here (".local/share/rune/skills") would bind skills to the process
			// CWD, so signal "no skills dir" and let the caller handle it.
			return ""
		}
		base = filepath.Join(home, ".local", "share")
	}
	return filepath.Join(base, "rune", "skills")
}

// AgentsDir returns ~/.agents/skills when that path exists and is a directory.
// It is a shared, read-only multi-agent skills root (Rune, Hermes, Claude Code,
...
// os.UserHomeDir(). RUNE_SKILLS_DIR is intentionally ignored — agents is a
// pure convention path, not a Rune-specific override.
