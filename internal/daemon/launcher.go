...
// reentrancyMarkers are the env vars rune sets on a command it has ALREADY
// wrapped in a sandbox. The daemon MUST strip them from every worker's
// environment: a worker that inherited them would trip rune's re-entrancy guard
...
// other variable is preserved — including the RUNE_SANDBOX_* policy config the
...
// execWorker is a WorkerHandle backed by a `rune exec` child process speaking
...
type ExecLauncherConfig struct {
	// Executable is the rune binary to spawn (defaults to os.Executable()).
	Executable string
...
// NewExecLauncher builds a Launcher that spawns `rune exec` workers which speak
...
func NewExecLauncher(cfg ExecLauncherConfig) (Launcher, error) {
	...
	if exe == "" {
		...
		if err != nil {
			return nil, fmt.Errorf("daemon: locate rune executable: %w", err)
		}
		exe = resolved
	}
