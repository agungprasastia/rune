// Package daemon implements rune's long-running daemon/server mode: a local
// control server that supervises a pool of headless `rune exec` worker processes
// and routes multiple agent sessions to them over an owner-only local socket.
//
// It reuses rune's existing building blocks rather than inventing new ones:
...
//   - internal/cli (exec) : a worker is a `rune exec -i/-o stream-json` process.
//
