...
// $XDG_RUNTIME_DIR/rune when set (a tmpfs owned by the user on Linux), otherwise
// ~/.rune. Mirrors supervisor.js / config.js choosing a per-user runtime dir.
func DefaultDir() (string, error) {
	if rt := strings.TrimSpace(os.Getenv("XDG_RUNTIME_DIR")); rt != "" {
		return filepath.Join(rt, "rune"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".rune"), nil
}
