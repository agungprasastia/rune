...
func TestKeyringGetSurfacesNonNotFoundError(t *testing.T) {
	...
	// non-rune exit is a real failure that must surface, not be masked as absent.
	k := &Keyring{
		goos: "darwin",
	...
	if _, ok, err := k.Get("rune", "tokens"); err == nil || ok {
...
func TestKeyringRoundTripDarwin(t *testing.T) {
	...
	if err := k.Set("rune", "tokens", "blob-AAA"); err != nil {
		t.Fatalf("Set: %v", err)
	}
	got, ok, err := k.Get("rune", "tokens")
	if err != nil || !ok {
	...
	existed, err := k.Delete("rune", "tokens")
	if err != nil || !existed {
		t.Fatalf("Delete: existed=%v err=%v", existed, err)
	}
	if _, ok, _ := k.Get("rune", "tokens"); ok {
...
func TestKeyringRoundTripDarwinUsesStdin(t *testing.T) {
	f := newFake("darwin")
	k := f.keyring()
	if err := k.Set("rune", "tokens", "blob-CCC"); err != nil {
		t.Fatalf("Set: %v", err)
	}
	...
	wantStdin := "add-generic-password -U -s \"rune\" -a \"tokens\" -w \"blob-CCC\"\n"
...
func TestKeyringSetRejectsMultilineSecretOnDarwin(t *testing.T) {
	for _, secret := range []string{"line1\nline2", "line1\r\nline2", "trailing\n"} {
		f := newFake("darwin")
		k := f.keyring()
		if err := k.Set("rune", "tokens", secret); err == nil {
...
func TestKeyringSetRejectsOversizedSecretOnDarwin(t *testing.T) {
	f := newFake("darwin")
	k := f.keyring()
	if err := k.Set("rune", "tokens", strings.Repeat("a", 4000)); err != nil {
		t.Fatalf("Set(4000 bytes): %v", err)
	}
	f = newFake("darwin")
	k = f.keyring()
	if err := k.Set("rune", "tokens", strings.Repeat("a", 5000)); err == nil {
...
func TestKeyringRoundTripLinuxUsesStdin(t *testing.T) {
	f := newFake("linux")
	k := f.keyring()
	if err := k.Set("rune", "tokens", "blob-BBB"); err != nil {
		t.Fatalf("Set: %v", err)
	}
...
func TestKeyringGetMissingIsNotError(t *testing.T) {
	for _, goos := range []string{"darwin", "linux"} {
		...
		if _, ok, err := k.Get("rune", "absent"); err != nil || ok {
			t.Fatalf("[%s] Get(absent) = ok=%v err=%v, want false/nil", goos, ok, err)
		}
		if existed, err := k.Delete("rune", "absent"); err != nil || existed {
...
func TestKeyringUnsupportedPlatform(t *testing.T) {
	...
	if err := k.Set("rune", "tokens", "x"); err == nil {
		t.Fatal("Set on unsupported platform should error")
	}
	if _, _, err := k.Get("rune", "tokens"); err == nil {
		t.Fatal("Get on unsupported platform should error")
	}
	if _, err := k.Delete("rune", "tokens"); err == nil {
...
func TestKeyringMissingBinaryError(t *testing.T) {
	...
	if err := k.Set("rune", "tokens", "x"); err == nil || !strings.Contains(err.Error(), "secret-tool") {
