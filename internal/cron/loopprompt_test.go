...
func TestResolveLoopPromptPriority(t *testing.T) {
	...
	// home/.rune/loop.md is used when cwd has none
	mustWrite(t, filepath.Join(home, ".rune", "loop.md"), "HOME PROMPT")
	if got, _ = ResolveLoopPrompt(cwd, home); got != "HOME PROMPT" {
		t.Fatalf("home loop.md not used, got %q", got)
	}
	// cwd/.rune/loop.md wins over home
	mustWrite(t, filepath.Join(cwd, ".rune", "loop.md"), "CWD PROMPT")
...
func TestResolveLoopPromptRejectsSymlinkAndCap(t *testing.T) {
	...
	home := t.TempDir()
	// oversize file -> error
	big := filepath.Join(cwd, ".rune", "loop.md")
	...
	link := filepath.Join(cwd2, ".rune", "loop.md")
