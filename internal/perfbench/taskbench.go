...
// RUNE must satisfy. The set is recorded by ID with every result so a published
...
// default runner executes to decide pass/fail after RUNE finishes. Class groups
...
type TaskConfig struct {
	...
	// production runner (NewExecRunner) invokes headless `rune exec`.
	Runner TaskRunner
}
...
func FormatTaskSummary(result TaskRunResult) string {
	...
	lines := []string{
		"Rune task benchmark: " + displayOrUnknown(result.Suite),
...
// NewExecRunner builds the production runner: it invokes headless `rune exec`
// with stream-json output for each task and decides pass/fail. binary is the path
// to the `rune` binary; extraArgs are appended to every invocation (e.g. sandbox
...
//
// Pass/fail is decided from the stream-json run_end event's exit code: a rune
...
func NewExecRunner(binary string, extraArgs ...string) TaskRunner {
	return func(ctx context.Context, task BenchTask, rc RunContext) TaskOutcome {
		...
		// The terminal run_end exit code is authoritative for pass/fail: a non-rune
		// agent exit is a normal task failure, not a harness error, even though
		// cmd.Run() reports any non-rune process exit as an *exec.ExitError. Only when
		...
		if !haveExit {
			...
			return TaskOutcome{Err: fmt.Errorf("rune exec failed: %s", detail)}
		}
...
// run_end exit code and the final event's text. RUNE emits one JSON object per
...
// and returns its text (the agent's final answer). RUNE emits one final event
// per run on both the success and incomplete paths (exec.go writer.final), so
// the last one wins. The turn benchmark writes this to .rune-answer.txt in the
