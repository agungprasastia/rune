...
```sh
make baseline RUNE_BENCH_MODEL=<model>              # uses ./rune
make baseline RUNE_BENCH_MODEL=<model> RUNE_BENCH_BINARY=/path/to/rune
```

This builds `rune`, then runs `rune-perf-bench turn` over
`internal/perfbench/manifests/baseline.json`, capturing each turn's trace
(`rune exec --trace <tmpfile>`) and writing the aggregated result to
...
- Each task is a fresh `rune exec` process, so iterations are **cold-start**
