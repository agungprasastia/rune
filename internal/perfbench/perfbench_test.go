...
func TestFormatSummaryIncludesWarnings(t *testing.T) {
	...
	result := Result{
		...
		ColdStartCommand:    []string{"/repo/rune", "--version"},
		FirstOutputCommand:  []string{"/repo/rune", "--version"},
		Iterations:          2,
		WarmupIterations:    1,
...
func TestPerfBenchHelperProcess(t *testing.T) {
	if os.Getenv("RUNE_PERF_HELPER") != "1" {
		return
	}
	fmt.Println("rune 0.1.0")
	os.Exit(0)
}
