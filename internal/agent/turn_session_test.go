...
func (s *fakeTurnSession) Stream(ctx context.Context, request runeruntime.CompletionRequest) (<-chan runeruntime.StreamEvent, error) {
	s.streams++
	return s.inner.StreamCompletion(ctx, request)
}

func (s *fakeTurnSession) Compact(context.Context, runeruntime.CompletionRequest) ([]runeruntime.Message, error) {
	return nil, runeruntime.ErrCompactionUnsupported
}
...
func (p *fakeTurnSessionProvider) OpenTurnSession(context.Context) (runeruntime.TurnSession, error) {
	p.opens++
	if p.openErr != nil {
...
func (p *fakeTurnSessionProvider) Capabilities() runeruntime.ProviderCapabilities {
	return runeruntime.ProviderCapabilities{}
}

func singleAnswerTurns(answer string) [][]runeruntime.StreamEvent {
...
func TestRunDefaultTurnSessionMatchesRawProvider(t *testing.T) {
	run := func(explicit bool) (Result, *mockProvider) {
		...
		if explicit {
			options.TurnSessionProvider = runeruntime.NewProviderTurnSessionProvider(provider, runeruntime.ProviderCapabilities{})
		}
...
func TestRunModelSessionSwitcherOpenFailureIsNonFatal(t *testing.T) {
	...
	result, err := Run(context.Background(), "go", inner, Options{
		...
		ModelSessionSwitcher: func(_ context.Context, _ string) (runeruntime.TurnSessionProvider, error) {
	...
	for _, message := range inner.requests[1].Messages {
		if message.Role == runeruntime.MessageRoleUser && strings.Contains(strings.ToLower(message.Content), "could not switch") {
			sawNote = true
		}
	}
	if !sawNote {
...
func TestRunModelSessionSwitcherErrorIsNonFatal(t *testing.T) {
	...
	for _, request := range inner.requests[1].Messages {
		if request.Role == runeruntime.MessageRoleUser && strings.Contains(strings.ToLower(request.Content), "could not switch") {
			sawNote = true
		}
	}
	if !sawNote {
