...
type imageEchoProvider struct {
	seen []runeruntime.Message
}

func (p *imageEchoProvider) StreamCompletion(ctx context.Context, request runeruntime.CompletionRequest) (<-chan runeruntime.StreamEvent, error) {
	if p.seen == nil {
		p.seen = append([]runeruntime.Message{}, request.Messages...)
	}
	events := make(chan runeruntime.StreamEvent)
	close(events)
	return events, nil
}
...
func TestRunSeedsImagesIntoUserTurn(t *testing.T) {
	...
	images := []runeruntime.ImageBlock{{MediaType: "image/png", Data: []byte{0x89, 0x50}}}
	...
	if user.Role != runeruntime.MessageRoleUser {
