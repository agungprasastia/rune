...
// reads.
func buildPromptSubstrings(options Options, exposed []runeruntime.ToolDefinition) promptSubstrings {
	return buildPromptSubstringsFromParts(buildSystemPromptParts(options), exposed)
}

func buildPromptSubstringsFromParts(parts systemPromptParts, exposed []runeruntime.ToolDefinition) promptSubstrings {
...
func toolSubstrings(exposed []runeruntime.ToolDefinition) (toolsSubstr, schemaSubstr string) {
	if len(exposed) == 0 {
		return "", ""
	}
...
func ComputePrefixFingerprint(options Options, exposed []runeruntime.ToolDefinition) prefixFingerprint {
