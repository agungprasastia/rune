...
func planUpdateTurn(status string) runeruntime.CollectedStream {
	return runeruntime.CollectedStream{ToolCalls: []runeruntime.ToolCall{
...
func TestPlanReminderFiresOnTurnStaleness(t *testing.T) {
	...
	for range stalePlanTurnThreshold {
		state.observeTurn(runeruntime.CollectedStream{Text: "still working…"})
	}
...
func TestPlanReminderSkipsTurnStalenessWhenComplete(t *testing.T) {
	...
	for range stalePlanTurnThreshold + 3 {
		state.observeTurn(runeruntime.CollectedStream{Text: "wrapping up…"})
	}
...
func TestPlanUpdateResetsTurnStaleness(t *testing.T) {
	...
	for range stalePlanTurnThreshold - 1 {
		state.observeTurn(runeruntime.CollectedStream{Text: "working…"})
	}
