...
func TestBrowserRunUsesHelperManifest(t *testing.T) {
	...
	t.Setenv(EnvHelperManifest, `{"version":1,"helpers":{"agent-browser":{"command":`+quoteJSON(helper)+`,"pathPrepend":[`+quoteJSON(binDir)+`],"env":{"RUNE_HELPER_TEST":"1"}}}}`)
	...
	if !envContains(runner.env, "RUNE_HELPER_TEST=1") {
		t.Fatalf("env = %#v, want RUNE_HELPER_TEST", runner.env)
	}
...
func TestBrowserRunUsesManifestPrefixArgs(t *testing.T) {
	runner := &fakeCommandRunner{}
	t.Setenv(EnvHelperManifest, `{"version":1,"helpers":{"agent-browser":{"command":"cmd.exe","prefixArgs":["/d","/s","/c","C:\\rune\\agent-browser.cmd"]}}}`)
	browser := NewBrowser(BrowserOptions{
		Enabled: true,
		Runner:  runner,
	})
	...
	want := []string{"/d", "/s", "/c", `C:\rune\agent-browser.cmd`, "open", "https://example.com"}
...
func TestMergeEnvReplacesPathCaseInsensitivelyOnWindows(t *testing.T) {
	...
	env := mergeEnv([]string{`Path=C:\Windows`, "RUNE=1"}, []string{`PATH=C:\Rune`})
	pathCount := 0
	for _, item := range env {
		...
		pathCount++
		if value != `C:\Rune` {
			t.Fatalf("PATH value = %q, want C:\\Rune", value)
		}
	}
	if pathCount != 1 {
...
func TestBrowserRunUsesAdjacentPackagedHelper(t *testing.T) {
	root := t.TempDir()
	native := filepath.Join(root, "rune")
