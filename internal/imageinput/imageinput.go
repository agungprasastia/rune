...
func LoadFile(path string, workspaceRoot string) (runeruntime.ImageBlock, error) {
	resolved := path
	...
	if err != nil {
		return runeruntime.ImageBlock{}, fmt.Errorf("image file not found: %s", path)
	}
	...
	if !info.Mode().IsRegular() {
		return runeruntime.ImageBlock{}, fmt.Errorf("image file must be a regular file: %s", path)
	}
	if info.Size() > MaxImageBytes {
		return runeruntime.ImageBlock{}, fmt.Errorf("image %s is larger than the 10 MiB limit", path)
	}
	...
	if err != nil {
		...
		return runeruntime.ImageBlock{}, fmt.Errorf("cannot open image %s: %w", path, err)
	}
	defer file.Close()

	data, err := io.ReadAll(io.LimitReader(file, MaxImageBytes+1))
	if err != nil {
		return runeruntime.ImageBlock{}, fmt.Errorf("cannot read image %s: %w", path, err)
	}
	...
	if mediaType == "" {
		return runeruntime.ImageBlock{}, fmt.Errorf("unsupported image type for %s (allowed: png, jpeg, gif, webp)", path)
	}

	return runeruntime.ImageBlock{MediaType: mediaType, Data: data}, nil
}
