...
func rasterizeWithPoppler(data []byte, maxPages int) ([]runeruntime.ImageBlock, error) {
	...
	images := make([]runeruntime.ImageBlock, 0, len(entries))
...
func loadRenderedPage(name string) (runeruntime.ImageBlock, error) {
	info, err := os.Stat(name)
	if err != nil {
		return runeruntime.ImageBlock{}, err
	}
	if info.Size() > MaxImageBytes {
		return runeruntime.ImageBlock{}, fmt.Errorf("rendered page %s exceeds the per-image limit", filepath.Base(name))
	}
	...
	if mediaType == "" {
		return runeruntime.ImageBlock{}, fmt.Errorf("rendered page %s has an unsupported image type", filepath.Base(name))
	}
