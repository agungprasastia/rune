...
func TestDefaultResolveOptionsUsesExistingConfigPathsAndProviderCommand(t *testing.T) {
	...
	userPath := filepath.Join(userConfigRoot, "rune", "config.json")
	projectPath := filepath.Join(workspaceRoot, ".rune", "config.json")
	writeFileAt(t, userPath, "{}")
	writeFileAt(t, projectPath, "{}")
	t.Setenv("RUNE_PROVIDER_COMMAND", "  rune-provider  ")
	...
	if options.ProviderCommand != "rune-provider" {
		t.Fatalf("ProviderCommand = %q, want rune-provider", options.ProviderCommand)
	}
}
...
func TestDefaultResolveOptionsRejectsDirectoryConfigPaths(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(userConfigRoot, "rune", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(workspaceRoot, ".rune", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
...
func TestDefaultUserConfigPathUsesXDGConfigOnMacOS(t *testing.T) {
	...
	want := filepath.Join(home, ".config", "rune", "config.json")
	if path != want {
