...
func (m model) fileViewNavBar(width int) string {
	...
	left := runeTheme.accent.Render("← "+truncatePathLeft(m.fileView.path, maxInt(8, width/2))) +
		runeTheme.faint.Render("  ·  "+mode)
	right := runeTheme.faint.Render(other + " · esc back")
...
func (m model) renderFileViewDiff(width int) string {
	...
	if len(rows) == 0 {
		return runeTheme.faint.Render("No recorded edits for this file in this session.")
	}
	...
	for i, row := range rows {
		...
		if len(rows) > 1 {
			b.WriteString(runeTheme.faint.Render(fmt.Sprintf("edit %d of %d", i+1, len(rows))))
			b.WriteString("\n")
		}
...
func (m model) renderFileViewFull(width int) string {
	...
	if err != nil {
		return runeTheme.faint.Render("Could not read file: " + err.Error())
	}
	defer file.Close()
	var lines []string
	truncated := false
	...
	for i, line := range display {
		...
		if changed[strings.TrimSpace(lines[i])] {
			marker = runeTheme.accent.Render("▎")
		}
		b.WriteString(runeTheme.faintest.Render(fmt.Sprintf("%*d ", gutterW, i+1)))
	...
	if truncated {
		...
		b.WriteString("\n")
		b.WriteString(runeTheme.faint.Render(fmt.Sprintf("… more lines (file truncated at %d for display)", len(lines))))
	}
	return b.String()
}
