...
func TestPrServiceDetectsGitHubPRAndDiffStats(t *testing.T) {
	...
	service := NewPrService("/workspace/rune")
...
func TestGetLocalDiffStatsFallsBackToOriginBase(t *testing.T) {
	...
	additions, deletions, err := getLocalDiffStats(context.Background(), "/workspace/rune", "main", runner.Run)
	if err != nil {
...
func TestTitleBarShowsPRDiffStatsBesideBranch(t *testing.T) {
	...
	for _, want := range []string{" feat/title-polish", "+144", "-32", "#219", "/workspace/rune"} {
