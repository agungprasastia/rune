...
func (m model) renderSpecialistCard(info specialistInfo, width int) string {
	...
	case specialistRunning:
		icon := m.spinnerGlyph()
		header = runeTheme.accent.Render(fmt.Sprintf("%s%s · %s · %s", icon, info.name, description, elapsedStr))
	case specialistCompleted:
		header = runeTheme.green.Render(fmt.Sprintf("✓ %s · %s · %s", info.name, description, elapsedStr))
	case specialistError:
		header = runeTheme.red.Render(fmt.Sprintf("✗ %s · %s · %s", info.name, description, elapsedStr))
	default:
		header = runeTheme.accent.Render(fmt.Sprintf("• %s · %s · %s", info.name, description, elapsedStr))
	}
	...
	var body string
	if info.status == specialistError {
		body = runeTheme.red.Render(bodyText)
	} else {
		body = runeTheme.muted.Render(bodyText)
	}
	...
	body += runeTheme.faint.Render("   · enter to open")
	...
	if info.status == specialistRunning && info.currentTool != "" {
		...
		lines = append(lines, runeTheme.muted.Render(progressLine))
	}
	...
	if info.status == specialistError && strings.TrimSpace(info.errorMsg) != "" {
		...
		lines = append(lines, runeTheme.red.Render("  "+errMsg))
	}
...
func specialistBorderStyle(status specialistStatus) lipgloss.Style {
	switch status {
	case specialistRunning:
		return runeTheme.cardRun
	case specialistError:
		return runeTheme.cardErr
	default:
		return runeTheme.line
	}
}
...
func renderSpecialistSummary(specialists []specialistInfo, spinnerView string) string {
	...
	return runeTheme.accent.Render(spinnerView) + runeTheme.muted.Render(summary[tailStart:])
}
