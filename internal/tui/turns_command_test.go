...
func TestHandleTurnsCommand(t *testing.T) {
	...
	t.Run("sets a valid budget and exports it for children", func(t *testing.T) {
		...
		if env := os.Getenv("RUNE_MAX_TURNS"); env != "150" {
			t.Fatalf("RUNE_MAX_TURNS = %q, want 150 (so sub-agents inherit it)", env)
		}
	})
	...
	t.Run("clamps above the ceiling (env too)", func(t *testing.T) {
		...
		if env := os.Getenv("RUNE_MAX_TURNS"); env != strconv.Itoa(maxTurnsCeiling) {
			t.Fatalf("RUNE_MAX_TURNS = %q, want clamped %d", env, maxTurnsCeiling)
		}
	})
