...
func TestSubmitThreadsImagesThenClears(t *testing.T) {
	...
	captured := make(chan []runeruntime.ImageBlock, 1)
	...
	m.captureRunImages = func(imgs []runeruntime.ImageBlock) { captured <- imgs }
...
func TestSubmitPrependsDocumentTextThenClears(t *testing.T) {
	...
	if last.Role != runeruntime.MessageRoleUser {
...
func TestRetryResendsAttachments(t *testing.T) {
	...
	m.lastImages = []runeruntime.ImageBlock{{MediaType: "image/png", Data: []byte{0x89, 'P', 'N', 'G'}}}
