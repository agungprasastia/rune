...
func TestPlainTranscriptTextSkipsNoise(t *testing.T) {
	...
	if !strings.Contains(out, "you: add a flag") || !strings.Contains(out, "rune: Done — added --version.") {
...
func TestHandleExportCommandWritesFile(t *testing.T) {
	...
	if !strings.Contains(string(data), "you: hello") || !strings.Contains(string(data), "rune: hi there") {
	...
	if msg := m.handleExportCommand(""); !strings.Contains(msg, "rune-transcript-20260704-093000.txt") {
