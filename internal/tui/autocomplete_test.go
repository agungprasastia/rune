...
func TestEnterWithNoSuggestionStillSubmits(t *testing.T) {
	m := newModel(context.Background(), Options{})
	m.input.SetValue("hello rune") // plain prompt, no suggestions
	...
	if !transcriptContains(next.transcript, "hello rune") {
...
func TestSuggestionsSuppressedDuringSpecReview(t *testing.T) {
	...
	m.pendingSpecReview = &pendingSpecReviewPrompt{SpecID: "spec-1", SpecFilePath: ".rune/specs/spec-1.md"}
