...
func (m model) mcpManagerItems() []mcpManagerItem {
	...
	for _, item := range []mcpManagerItem{
		{Kind: mcpManagerItemAddRemote, Name: "custom-remote", Label: "Add MCP server", Meta: "rune mcp add <name> --url <url>", Detail: "custom remote http sse url"},
		{Kind: mcpManagerItemAddStdio, Name: "custom-stdio", Label: "Add local stdio MCP", Meta: "rune mcp add <name> -- <command> [args...]", Detail: "custom local stdio command"},
		{Kind: mcpManagerItemList, Name: "list", Label: "List configured", Meta: "rune mcp list", Detail: "configured installed servers"},
	} {
		if mcpManagerItemMatches(item, query) {
			items = append(items, item)
		}
...
func (m model) mcpManagerOverlay(width int) string {
	...
	state := m.mcpViewState()
	lines := []string{
		fillPaletteLine(runeTheme.ink.Bold(true).Render(pluralCount(len(state.Servers), "server")), innerWidth, transparentSurface),
		fillPaletteLine(renderMCPManagerSearchLine(m.mcpManager.query, innerWidth), innerWidth, transparentSurface),
		runeTheme.accent.Bold(true).Render("User MCPs"),
	}
	if len(state.Servers) == 0 {
		lines = append(lines, runeTheme.faint.Render("  No MCP servers configured."))
	}
	...
	lines = append(lines, runeTheme.line.Render(strings.Repeat("─", innerWidth)))
	lines = append(lines, fillPaletteLine(runeTheme.faint.Render("type search   up/down navigate   Enter action   Alt+d disable   Esc close"), innerWidth, transparentSurface))
	return centerRenderedBlock(styledBlockFillTitle(overlayWidth, "Manage MCP servers", lines, runeTheme.lineStrong, lipgloss.NewStyle()), width)
}
...
func renderMCPManagerSearchLine(query string, width int) string {
	...
	if query == "" {
		return fitStyledLine(prompt+runeTheme.faint.Render("MCP servers, tools, docs..."), width)
	}
	return fitStyledLine(prompt+runeTheme.ink.Render(query), width)
}

func (m model) renderMCPManagerItemLines(width int, items []mcpManagerItem) ([]string, []int) {
	if len(items) == 0 {
		return []string{fillPaletteLine(runeTheme.faint.Render("  no MCP actions"), width, transparentSurface)}, []int{-1}
	}
	...
	for offset, item := range visible {
		...
		if group := mcpManagerItemGroup(item.Kind); group != "" && group != lastGroup {
			lines = append(lines, runeTheme.accent.Bold(true).Render(group))
		...
		right := ""
		if item.Meta != "" {
			right = surface(runeTheme.faint).Render(item.Meta)
		}
...
func (m model) mcpManagerSelectionDetail(width int) []string {
	...
	case mcpManagerItemServer:
		...
		lines := []string{
			fillPaletteLine(runeTheme.ink.Bold(true).Render(server.Name)+" "+runeTheme.faint.Render(server.Transport+" · "+server.State), width, transparentSurface),
		}
		if target := strings.TrimSpace(server.Target); target != "" {
			lines = append(lines, fitStyledLine(runeTheme.faint.Render(target), width))
		}
		action := "Alt+d disable"
		if strings.EqualFold(strings.TrimSpace(server.State), "disabled") {
			action = "Alt+e enable"
		}
		lines = append(lines, fillPaletteLine(runeTheme.faint.Render("Enter check   Alt+c check   "+action+"   Alt+r remove"), width, transparentSurface))
		return lines
	case mcpManagerItemMarketplace:
		lines := []string{
			fillPaletteLine(runeTheme.ink.Bold(true).Render(item.Label)+" "+runeTheme.faint.Render(item.Meta), width, transparentSurface),
		}
		if detail := firstMCPMarketplaceDetailLine(item); detail != "" {
			lines = append(lines, fitStyledLine(runeTheme.faint.Render(detail), width))
		}
		lines = append(lines, fillPaletteLine(runeTheme.faint.Render("Enter fills composer: "+item.InstallCommand), width, transparentSurface))
		return lines
	case mcpManagerItemAddRemote:
		return []string{fillPaletteLine(runeTheme.faint.Render("Enter fills composer: /mcp add <name> --url <url>"), width, transparentSurface)}
	case mcpManagerItemAddStdio:
		return []string{fillPaletteLine(runeTheme.faint.Render("Enter fills composer: /mcp add <name> -- <command> [args...]"), width, transparentSurface)}
	case mcpManagerItemList:
		return []string{fillPaletteLine(runeTheme.faint.Render("Enter runs rune mcp list and refreshes this manager."), width, transparentSurface)}
	default:
		return nil
	}
}
