...
func TestProviderCommandShowsConfiguredOnboardingActions(t *testing.T) {
	...
	for _, want := range []string{
		...
		"active: yes",
		"api key: set",
		"rune providers check openai --connectivity",
...
func TestProviderCommandShowsMissingCredentialAction(t *testing.T) {
	...
	for _, want := range []string{
		...
		"rune providers add anthropic --api-key-env ANTHROPIC_API_KEY --set-active",
		"rune providers check anthropic --connectivity",
	} {
		assertContains(t, text, want)
	}
}
...
func TestProviderCommandShowsMissingCredentialActionForCompatibleProvider(t *testing.T) {
	...
	for _, want := range []string{
		...
		"rune providers add custom-openai-compatible --api-key-env OPENAI_API_KEY --set-active",
	} {
		assertContains(t, text, want)
	}
}
