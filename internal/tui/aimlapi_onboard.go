...
func (s *aimlapiOnboardState) view(width int, spinner string) []string {
	...
	if s.errText != "" {
		lines = append(lines, blankSetupBlockLine(rowWidth))
		lines = append(lines, aimlapiBlockText(s.errText, runeTheme.red.Render, rowWidth)...)
	}
	return lines
}
...
func aimlapiSpinnerLine(spinner string, width int) string {
	...
	return padSetupLine("  "+runeTheme.accent.Render(glyph), width)
}
...
func aimlapiPromptLines(prompt string, width int) []string {
	...
	for index, promptLine := range strings.Split(prompt, "\n") {
		render := runeTheme.ink.Render
		if index > 0 {
			render = runeTheme.faint.Render
		}
...
func aimlapiInputScreen(width int, prompt, inputPrompt, value, placeholder, footnote string) []string {
	...
	if footnote != "" {
		lines = append(lines, aimlapiBlockText(footnote, runeTheme.faint.Render, width)...)
	}
	return lines
}
...
func aimlapiOptionRows(label, hint string, selected bool, width int) []string {
	...
	if hint != "" {
		rows = append(rows, padSetupLine("    "+runeTheme.faint.Render(hint), width))
	}
	return rows
}
...
func (s *aimlapiOnboardState) viewPickPath(width int) []string {
	...
	lines := []string{}
	lines = append(lines, aimlapiBlockText(aimlapi.MsgPickPathPrompt, runeTheme.ink.Render, rowWidth)...)
	lines = append(lines, blankSetupBlockLine(rowWidth))
...
func aimlapiFieldMarker(selected bool) string {
	if selected {
		return runeTheme.accent.Bold(true).Render("❯") + " "
	}
	return "  "
}
...
func aimlapiAmountInputLine(value string, placeholder string, width int, focused bool) string {
	promptStyle := runeTheme.ink
	if focused {
		promptStyle = runeTheme.accent
	}
	prompt := promptStyle.Render("amount > ") + runeTheme.ink.Render("$")
...
func aimlapiToggleLine(prompt string, on bool, focused bool) string {
	...
	onText, offText := runeTheme.faint.Render("on"), runeTheme.faint.Render("off")
	if on {
		onText = runeTheme.ink.Render("on")
	} else {
		offText = runeTheme.ink.Render("off")
	}
	return promptStyle.Render(prompt) + onText + runeTheme.ink.Render("/") + offText
}
...
func (s *aimlapiOnboardState) viewProgress(width int, spinner string) []string {
	...
	if link := strings.TrimSpace(s.detail); link != "" {
		lines = append(lines, aimlapiBlockText("Opening checkout in browser...", runeTheme.ink.Render, rowWidth)...)
		lines = append(lines, blankSetupBlockLine(rowWidth))
		lines = append(lines, aimlapiBlockText(fmt.Sprintf(aimlapi.MsgTopUpBrowserFallback, ""), runeTheme.faint.Render, rowWidth)...)
		lines = append(lines, blankSetupBlockLine(rowWidth))
...
func aimlapiLinkLines(link string, width int) []string {
	...
	if schemeEnd := strings.Index(remaining, "://"); schemeEnd > 0 {
		...
		lines = append(lines, padSetupLine("  "+runeTheme.accent.Render(head), width))
...
func (s *aimlapiOnboardState) viewDone(width int) []string {
	...
	for _, line := range s.successLines {
		...
		if line == aimlapi.MsgEverythingRuns || line == aimlapi.MsgTopUpSuccess ||
			...
			render = runeTheme.accent.Render
		}
		lines = append(lines, aimlapiBlockText(line, render, rowWidth)...)
	}
	lines = append(lines, blankSetupBlockLine(rowWidth))
	lines = append(lines, aimlapiBlockText("Press Enter to continue.", runeTheme.faint.Render, rowWidth)...)
	return lines
}
