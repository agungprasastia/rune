...
func (m model) streamingToolCallView(width int) string {
	...
	head := toolCardHead(m.streamCallName, d.path, "", headTag, "", "", true, runeTheme.ink, false, width, cardRenderOptions{cwd: m.cwd})
	...
	return renderLeftRuleCard(width, []string{head}, runeTheme.cardRun)
}
