...
func TestBrowserClickTypeAndPressRunStructuredCommands(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "type",
			args: map[string]any{"ref": "e114", "text": "hey from rune"},
			want: []string{"type", "e114", "hey from rune"},
		},
		{
			name: "press",
...
func TestBrowserTypeRejectsMissingRefBeforePermission(t *testing.T) {
	...
	result, rejected := tool.(PrePermissionRejecter).RejectBeforePermission(map[string]any{"text": "hey from rune"})
