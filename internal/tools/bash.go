...
func (tool bashTool) run(ctx context.Context, args map[string]any, engine *zeroSandbox.Engine, directBudget bool) Result {
	...
	// can't grow Rune's memory before truncation: only the head+tail each stream
...
func formatBashOutput(stdout string, stderr string, exitCode int) string {
	...
	if n := len(outFindings) + len(errFindings); n > 0 {
		parts = append(parts, fmt.Sprintf("[rune] redacted %d likely secret(s) from this output before showing it.", n))
	}
	if len(parts) == 0 {
...
func budgetBashCapture(out string, outTotal int, errStr string, errTotal int, meta map[string]string) (string, string, bool) {
	...
	if truncated {
		if spillPath := spillBashStreams(out, outTotal, errStr, errTotal); spillPath != "" {
			hint := "\n[rune] captured output saved to " + spillPath + " (grep or read_file it instead of re-running)"
			if errTrunc {
				errText += hint
			} else {
				outText += hint
...
// tail per stream — everything rune kept in memory, which is more than the
...
func sectionWithCaptureGap(text string, total int) string {
	...
	marker := fmt.Sprintf("\n[rune] capture gap: %d bytes omitted from the middle of this stream\n", total-len(text))
...
// a command emitting unbounded output (`cat huge.log`, `yes`) cannot grow Rune's
...
func truncateHeadTailWithTotal(value string, total, maxBytes int) (string, int, bool) {
	...
	marker := fmt.Sprintf("\n[rune] output truncated: %d bytes omitted from the middle — redirect to a file and read_file a range for the full text\n", total-maxBytes)
