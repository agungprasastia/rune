...
func TestWebFetchToolFetchesHTTPText(t *testing.T) {
	tool := newWebFetchToolWithClient(webFetchTestClient(func(request *http.Request) (*http.Response, error) {
		...
		return webFetchTestResponse(request, http.StatusOK, "text/plain; charset=utf-8", "hello rune"), nil
	}))
	...
	for _, want := range []string{"URL: https://example.com/guide?token=[REDACTED]", "Status: 200 OK", "Content-Type: text/plain; charset=utf-8", "hello rune"} {
