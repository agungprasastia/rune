...
func (tool writeFileTool) RunWithOptions(ctx context.Context, args map[string]any, options RunOptions) Result {
	...
	// Rune since it was last read — the new content was likely composed against a
	...
	// Optional format-on-write (RUNE_FORMAT_ON_WRITE). Must run BEFORE the
	...
	// re-reads the file — the rich preview costs rune model tokens.
