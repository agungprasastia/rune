...
func shellStub(name string, description string) string {
	return `#!/bin/sh
# ` + name + ` — Rune plugin tool entry point.
# ` + description + `
#
# Rune invokes this script with the tool-call arguments as a single JSON object
...
func nodeStub(name string, description string) string {
	return `// ` + name + ` — Rune plugin tool entry point.
// ` + description + `
//
// Rune invokes this script with the tool-call arguments as a single JSON object
// on stdin and expects the tool result on stdout.
...
Rune invokes this script with the tool-call arguments as a single JSON object on
