...
// session — i.e. it was modified outside Rune. The write tools surface it with
...
// content against this baseline to detect a modification made outside Rune.
//
...
type fileObservation struct {
	...
	ranges     []lineRange
	totalBytes int
	byteRanges []lineRange // rune-based, half-open byte intervals
}
...
func fileConflictMessage(relativePath string) string {
	return "Error writing " + relativePath + ": " + ErrFileChangedOnDisk.Error() +
		" (it may have been edited outside Rune). Re-read it with read_file, then re-apply your change so you do not overwrite the newer content."
}
