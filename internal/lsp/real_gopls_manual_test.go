...
// gopls on PATH (issue #684). So it runs only when RUNE_GOPLS_INTEGRATION=1,
...
// RUNE_GOPLS_INTEGRATION=0 keeps the integration check OFF, as its value implies.
// The default suite keeps its coverage on the fake server.
func TestManagerCheckRealGopls(t *testing.T) {
	if os.Getenv("RUNE_GOPLS_INTEGRATION") != "1" {
		t.Skip("set RUNE_GOPLS_INTEGRATION=1 to run the real gopls integration check")
	}
