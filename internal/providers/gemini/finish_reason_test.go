...
func TestMapFinishReasonNonNormal(t *testing.T) {
	...
	for _, cf := range []string{"SAFETY", "RECITATION", "IMAGE_SAFETY", "PROHIBITED_CONTENT", "BLOCKLIST", "SPII"} {
		if got := mapFinishReason(cf); got != runeruntime.FinishReasonContentFilter {
			t.Errorf("%q → %q, want content_filter (M3)", cf, got)
		}
	}
	if got := mapFinishReason("MAX_TOKENS"); got != runeruntime.FinishReasonLength {
...
func TestStreamCompletionSurfacesRecitationFinishReason(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == runeruntime.StreamEventDone {
			sawDone = true
			if e.FinishReason != runeruntime.FinishReasonContentFilter {
				t.Fatalf("RECITATION done FinishReason = %q, want %q (M3)", e.FinishReason, runeruntime.FinishReasonContentFilter)
			}
		}
	}
	if !sawDone {
...
func TestStreamCompletionDoesNotDropValidFunctionCall(t *testing.T) {
	...
	for _, e := range events {
		...
		if e.Type == runeruntime.StreamEventToolCallStart && e.ToolName == "read_file" {
			sawStart = true
		}
	}
	if !sawStart {
