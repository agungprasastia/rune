...
// Override globally with RUNE_STREAM_IDLE_TIMEOUT.
...
// 6 minutes; it still scales with RUNE_STREAM_IDLE_TIMEOUT. A returned value <= 0
...
func ContentStallTimeout(idleTimeout time.Duration) time.Duration {
	...
	// pathologically large RUNE_STREAM_IDLE_TIMEOUT could make idle*6 overflow
...
const streamIdleTimeoutEnv = "RUNE_STREAM_IDLE_TIMEOUT"
...
// RUNE_STREAM_IDLE_TIMEOUT env override if set and valid; otherwise
...
//     delivers real bytes, just at a crippled rate, resetting Rune's stream
...
func ClassifiedError(statusCode int, message string, secrets ...string) string {
	...
	case http.StatusUnauthorized, http.StatusForbidden:
		...
		curated := "auth error: your API key is missing or invalid — run `rune setup`, `rune auth openrouter` for OpenRouter, or set the provider's API key, then retry."
