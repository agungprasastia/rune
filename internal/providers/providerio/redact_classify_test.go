...
func TestClassifiedErrorCuratesAuthFailure(t *testing.T) {
	...
	if !strings.Contains(msg, "rune setup") || !strings.Contains(msg, "rune auth openrouter") {
	...
	// 403 also curated.
	if got := ClassifiedError(http.StatusForbidden, "forbidden"); !strings.Contains(got, "rune setup") {
