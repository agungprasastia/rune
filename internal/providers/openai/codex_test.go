...
func drainCodexEvents(t *testing.T, stream <-chan runeruntime.StreamEvent) {
	t.Helper()
	for ev := range stream {
		if ev.Type == runeruntime.StreamEventError {
...
func TestCodexProviderStreamsReasoningSummaryDeltas(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), runeruntime.CompletionRequest{
		Messages:        []runeruntime.Message{{Role: runeruntime.MessageRoleUser, Content: "solve it"}},
		ReasoningEffort: "high",
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
	...
	for ev := range stream {
		switch ev.Type {
		case runeruntime.StreamEventReasoning:
			reasoning = append(reasoning, ev.Content)
		case runeruntime.StreamEventText:
			text = append(text, ev.Content)
		case runeruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
		}
	}
...
func TestCodexProviderParsesResponsesTextDeltas(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), runeruntime.CompletionRequest{
		Messages: []runeruntime.Message{{Role: runeruntime.MessageRoleUser, Content: "say hi"}},
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
	}
	var texts []string
	var usage *runeruntime.Usage
	gotDone := false
	for ev := range stream {
		switch ev.Type {
		case runeruntime.StreamEventText:
			texts = append(texts, ev.Content)
		case runeruntime.StreamEventUsage:
			usage = &ev.Usage
		case runeruntime.StreamEventDone:
			gotDone = true
		case runeruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
		}
...
func TestCodexProviderParsesResponsesToolCalls(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), runeruntime.CompletionRequest{
		Messages: []runeruntime.Message{{Role: runeruntime.MessageRoleUser, Content: "weather in SF?"}},
		Tools: []runeruntime.ToolDefinition{
	...
	for ev := range stream {
		...
		case runeruntime.StreamEventToolCallDelta:
			deltas++
			fragments = append(fragments, ev.ArgumentsFragment)
		case runeruntime.StreamEventToolCallEnd:
			ends++
			endName = ev.ToolName
		case runeruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
...
func TestCodexProviderSendsAssistantToolCallsAsInputItems(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), runeruntime.CompletionRequest{
		Messages: []runeruntime.Message{
			{Role: runeruntime.MessageRoleUser, Content: "weather in SF?"},
			{
			...
			{Role: runeruntime.MessageRoleTool, ToolCallID: "call-1", Content: "72F and sunny"},
			{Role: runeruntime.MessageRoleUser, Content: "thanks"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
...
func TestCodexProviderEmitsLengthFinishWhenStreamEndsWithoutCompletion(t *testing.T) {
	...
	var gotDone bool
	for ev := range stream {
		if ev.Type == runeruntime.StreamEventDone {
			gotDone = true
	...
	if finishReason != runeruntime.FinishReasonLength {
		t.Fatalf("finish reason = %q, want %q", finishReason, runeruntime.FinishReasonLength)
	}
}
