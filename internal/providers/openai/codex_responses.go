...
func (p *CodexProvider) buildResponsesRequest(request runeruntime.CompletionRequest) (*responsesRequest, error) {
...
func (p *CodexProvider) userInputItem(msg runeruntime.Message) inputItem {
	parts := []contentItem{}
	if msg.Content != "" {
...
func (p *CodexProvider) assistantInputItems(msg runeruntime.Message) ([]inputItem, error) {
	items := []inputItem{}
	if msg.Content != "" {
...
func (p *CodexProvider) streamResponses(
	...
	if !state.done {
		...
		providerio.SendEvent(ctx, events, runeruntime.StreamEvent{
			...
			FinishReason: runeruntime.FinishReasonLength,
		})
	}
}
...
func (p *CodexProvider) emitResponsesEvent(
	...
	case responsesEventOutputTextDelta:
		if event.Delta != "" {
			providerio.SendEvent(ctx, events, runeruntime.StreamEvent{
				Type:    runeruntime.StreamEventText,
				Content: event.Delta,
			})
		}
		return true
...
func (p *CodexProvider) handleTerminalResponse(
	...
	if event.Response == nil {
		...
		if event.Type == responsesEventFailed {
			...
			providerio.SendEvent(ctx, events, runeruntime.StreamEvent{Type: runeruntime.StreamEventDone})
		}
		state.done = true
		return false
	}
