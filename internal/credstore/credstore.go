...
// headless/CI machines, so it is opt-in via RUNE_CRED_STORAGE=keyring). A plaintext
// file is available only as an explicit RUNE_CRED_STORAGE=file opt-out. OAuth tokens
...
type Options struct {
	...
	// (plaintext opt-out). When empty it falls back to RUNE_CRED_STORAGE, then auto:
	...
	// users can still opt into the keyring with RUNE_CRED_STORAGE=keyring.
	Storage string
...
func New(options Options) (*Store, error) {
	...
	if storage == "" {
		storage = strings.TrimSpace(env("RUNE_CRED_STORAGE"))
	}
	if storage == "" {
		...
		// encrypted file; keyring stays available via an explicit RUNE_CRED_STORAGE.
