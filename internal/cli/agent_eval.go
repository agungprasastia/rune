...
func writeAgentEvalHelp(w io.Writer) error {
  ...
  rune eval --suite <path> [flags]
  rune eval run --suite <path> [flags]
  rune eval bench --suite <path> [flags] [--model <id>] [--agent-command <argv...>]
...
func agentEvalBenchWorkRoot(options agentEvalOptions) (string, string, error) {
	...
	created, err := os.MkdirTemp("", "rune-eval-")
	if err != nil {
