...
// projectMCPConfigExists reports whether the workspace's project ./.rune/config.json
...
func projectMCPConfigExists(workspaceRoot string) bool {
	...
	data, err := os.ReadFile(filepath.Join(workspaceRoot, ".rune", "config.json"))
	if err != nil {
		return false
	}
...
func runMCPRemove(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: rune mcp remove <server> [--json]")
	}
...
func runMCPToggle(args []string, stdout io.Writer, stderr io.Writer, deps appDeps, disabled bool) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, fmt.Sprintf("usage: rune mcp %s <server> [--json]", commandName))
	}
	serverName := positional[0]
	if err := mcp.ValidateServerName(serverName); err != nil {
		return writeExecUsageError(stderr, err.Error())
...
func runMCPCheck(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: rune mcp check <server> [--json]")
	}
	serverName := positional[0]
	if err := mcp.ValidateServerName(serverName); err != nil {
		return writeExecUsageError(stderr, err.Error())
	...
	// so a cloned repo cannot have `rune mcp check <its-server>` run its command. No
	...
	if !ok {
		...
		// a dropped project server reads as "run rune trust", not a bare miss. The notice
	...
	// best-effort so one unreachable server cannot stop Rune launching: it records
...
func parseMCPAddArgs(args []string) (mcpAddOptions, bool, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case options.serverName == "":
			if strings.HasPrefix(arg, "-") {
				return options, false, execUsageError{"usage: rune mcp add <server> [flags] -- <command> [args...]"}
			}
	...
	case mcp.ServerTypeHTTP, mcp.ServerTypeSSE:
		...
		if strings.TrimSpace(options.server.URL) == "" {
			return options, false, execUsageError{fmt.Sprintf("rune mcp add --type %s requires --url", options.server.Type)}
		}
		if len(command) > 0 {
			return options, false, execUsageError{fmt.Sprintf("rune mcp add --type %s does not accept a command", options.server.Type)}
		}
	default:
...
func writeMCPWritableConfig(path string, cfg mcpWritableConfig) error {
	...
	tmp, err := os.CreateTemp(dir, ".rune-config-*.tmp")
	if err != nil {
...
func (cfg *mcpWritableConfig) setServerDisabled(name string, disabled bool) (bool, bool, error) {
	...
	if !found {
		...
		case config.IsDefaultMCPServer(name):
			...
			// letting `rune mcp disable <default>` work even though it lives in code.
			raw = nil
		default:
...
func writeMCPAddHelp(w io.Writer) error {
  ...
  rune mcp add <server> [flags] -- <command> [args...]
  rune mcp add <server> --url <url> [flags]
