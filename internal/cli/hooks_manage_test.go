...
func TestRunHooksAddPersistsToProjectConfig(t *testing.T) {
	...
	code := runHooksAdd([]string{"rune.preflight", "--event", "beforeTool", "--matcher", "bash", "--command", "sh", "--arg", "-c", "--arg", "echo hi"}, &stdout, &stderr, hooksManageDeps(cwd))
	if code != exitSuccess {
	...
	hook := config.Hooks[0]
	if hook.ID != "rune.preflight" || hook.Event != hooks.EventBeforeTool || hook.Command != "sh" {
