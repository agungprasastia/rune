...
// Distribution command handlers: `rune skill {add,info,remove}`,
// `rune plugin {add,remove}`, and `rune tools {make,list}`. These wire the
...
func runSkillAdd(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if source == "" {
		return writeExecUsageError(stderr, "usage: rune skill add <git-url|path> [--force] [--json]")
	}
	if dir == "" {
...
func runSkillInfo(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: rune skill info <name> [--json]")
	}
...
func runSkillRemove(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: rune skill remove <name>")
	}
...
func runPluginAdd(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if source == "" {
		return writeExecUsageError(stderr, "usage: rune plugin add <git-url|path> [--force] [--json]")
	}
	if dir == "" {
...
func runPluginRemove(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if id == "" {
		return writeExecUsageError(stderr, "usage: rune plugin remove <id>")
	}
...
func runPluginInfo(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if id == "" {
		return writeExecUsageError(stderr, "usage: rune plugins info <id> [--user] [--json]")
	}
...
func pluginLoadOptions(deps appDeps, cwd string, userOnly bool) plugins.LoadOptions {
	...
	if !userOnly {
		roots = append(roots, plugins.Root{Source: plugins.SourceProject, Path: filepath.Join(cwd, ".rune", "plugins")})
	}
	opts.Roots = roots
	return opts
}
...
func runTools(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "tools subcommand required. Use `rune tools make <name>` or `rune tools list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runToolsMake(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: rune tools make <name> [--runtime shell|node|python] [--description text]")
	}
	if dir == "" {
	...
	lines := []string{
		...
		"  2. Run `rune tools list` to confirm it loads.",
...
func formatToolboxList(items []toolboxItem, diagnostics []plugins.Diagnostic, dir string) string {
	lines := []string{}
	if len(items) == 0 {
		lines = append(lines, fmt.Sprintf("No Rune plugin-tools found in %s.", dir))
	} else {
		lines = append(lines, "Rune Tools:")
...
func writeSkillAddHelp(w io.Writer) error {
  ...
  rune skill add <git-url|path> [flags]
...
func writePluginAddHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  rune plugin add <git-url|path> [flags]
...
func writePluginInfoHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  rune plugins info <id> [flags]

Flags:
      --user    Target only user plugins (ignore ./.rune/plugins)
