...
func TestActivatePluginsRegistersToolAndCollectsHooks(t *testing.T) {
	...
	if activation.hooks[0].ID != "rune.demo.pre" {
		t.Fatalf("plugin hook id = %q, want namespaced rune.demo.pre", activation.hooks[0].ID)
	}
	if stderr.Len() != 0 {
...
func TestActivatePluginsAlwaysRegistersMultiRootSkillTool(t *testing.T) {
	// Even with rune plugins / rune plugin skill roots, activation must overlay
	...
	if res.Status != tools.StatusOK || !bytes.Contains([]byte(res.Output), []byte("agents body")) {
		t.Fatalf("agents skill must load with rune plugin roots: %q (%s)", res.Status, res.Output)
	}
}
