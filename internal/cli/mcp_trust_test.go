...
func TestMCPGateUntrustedExcludesProjectServer(t *testing.T) {
	...
	// This repo has no ./.rune/config.json, so there is no project MCP config to
...
// TestMCPToolsListSurfacesTrustNotice proves `rune mcp tools list` no longer drops the
...
func TestMCPToolsListSurfacesTrustNotice(t *testing.T) {
	...
	if errUntrusted := run(); !strings.Contains(errUntrusted, "MCP servers") || !strings.Contains(errUntrusted, "rune trust") {
...
// only a ./.rune/config.json that parses AND declares at least one server is true.
...
// untrusted workspace actually declares project MCP servers in ./.rune/config.json,
...
// TestMCPCheckSurfacesTrustNotice proves the `rune mcp check` notice fix: in an untrusted
// workspace whose only definition of a server lives in ./.rune/config.json, the gate
...
func TestMCPCheckSurfacesTrustNotice(t *testing.T) {
	...
	if got := errBuf.String(); !strings.Contains(got, "not configured") ||
		!strings.Contains(got, "MCP servers") || !strings.Contains(got, "rune trust") {
	...
	if got := errBuf2.String(); strings.Contains(got, "ignoring project") || strings.Contains(got, "rune trust") {
