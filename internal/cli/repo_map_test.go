...
func TestRunRepoMapHelpDocumentsFlags(t *testing.T) {
	...
	for _, want := range []string{"Usage:", "rune repo-map [flags]", "--json", "--query", "--max-files", "--scan-max-files", "--max-depth"} {
...
func TestRunRepoMapTextSummaryUsesCurrentWorkingDirectory(t *testing.T) {
	...
	for _, want := range []string{"Repo map", "Root: " + cwd, "Files:", "Languages:", "cmd/rune/main.go"} {
...
func TestRunRepoMapQueryPrintsRankedMatches(t *testing.T) {
	...
	if repoMapOutputContains(out, "cmd/rune/main.go") {
...
func newRepoMapFixture(t *testing.T) string {
	...
	writeRepoMapFixtureFile(t, dir, "cmd/rune/main.go", "package main\n\nfunc main() {}\n")
