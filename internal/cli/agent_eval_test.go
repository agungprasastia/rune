...
func TestRunEvalJSONMode(t *testing.T) {
	...
	for _, key := range []string{"tasks", "checks", "failed", "blocked", "errors"} {
		if string(raw[key]) != "0" {
			t.Fatalf("expected JSON key %q to be rune, got %s", key, string(raw[key]))
		}
	}
}
...
func TestRunEvalRunJSONModePassesRunnerOptions(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		"--task", "edit-reader",
		"--workspace", "D:\\work\\rune-fixture",
		"--json",
	}, &stdout, &stderr, appDeps{
		runAgentEval: func(ctx context.Context, options agentEvalOptions) (agentEvalReport, error) {
			if options.Mode != "run" || options.SuitePath != "evals/context.json" || options.TaskID != "edit-reader" || options.WorkspacePath != "D:\\work\\rune-fixture" || !options.JSON {
...
func TestRunEvalBenchJSONModePassesHarnessOptions(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		"--suite", "evals/context.json",
		"--task", "edit-reader",
		"--work-root", "D:\\tmp\\rune-evals",
		"--json",
		"--agent-command", "rune", "exec", "{prompt}",
	}, &stdout, &stderr, appDeps{
		runAgentEval: func(ctx context.Context, options agentEvalOptions) (agentEvalReport, error) {
			if options.Mode != "bench" || options.SuitePath != "evals/context.json" || options.TaskID != "edit-reader" || options.WorkRoot != "D:\\tmp\\rune-evals" || !options.JSON {
				t.Fatalf("unexpected eval bench options: %#v", options)
			}
			if got, want := strings.Join(options.AgentCommand, "\x00"), strings.Join([]string{"rune", "exec", "{prompt}"}, "\x00"); got != want {
				t.Fatalf("agent command = %#v, want rune exec {prompt}", options.AgentCommand)
			}
			return agentEvalReport{
				Suite:  "quality-context",
				TaskID: "edit-reader",
...
func TestRunEvalBenchTextOutputSurfacesWorkRoot(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "work-root: /tmp/rune-eval-abc") {
...
func TestRunEvalRunRejectsBenchOnlyFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name: "work root",
			args: []string{"eval", "run", "--suite", "evals/context.json", "--work-root", "D:\\tmp\\rune-evals"},
		...
		{
			name: "agent command",
			args: []string{"eval", "run", "--suite", "evals/context.json", "--agent-command", "rune", "exec", "{prompt}"},
...
func TestRunEvalDefaultRunnerLoadsSuite(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{
		...
		"tasks": [{
			...
			"workspaceFixture": "fixtures/rune",
			"verificationCommands": [
...
func TestRunEvalFailingSuiteReturnsProviderExit(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "Rune agent eval") || !strings.Contains(stdout.String(), "context.recall") {
