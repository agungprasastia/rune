...
// at <root>/.config/rune while the tests wrote to <root>/rune, so the store-error
...
// writeProjectHooks writes a project ./.rune/hooks.json under repo with the given
// hooks and top-level enabled flag.
func writeProjectHooks(t *testing.T, repo string, enabled bool, defs ...hooks.Definition) {
	t.Helper()
	path := filepath.Join(repo, ".rune", "hooks.json")
...
func writeUserHooks(t *testing.T, configRoot string, enabled bool, defs ...hooks.Definition) {
	t.Helper()
	path := filepath.Join(configRoot, "rune", "hooks.json")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
...
func TestPluginGateUntrustedExcludesProject(t *testing.T) {
	setTrustConfigRoot(t)
	repo := t.TempDir()
	// A ./.rune/plugins dir so the skip report flags an excluded project config.
	if err := os.MkdirAll(filepath.Join(repo, ".rune", "plugins"), 0o700); err != nil {
	...
	if !activation.excludedProjectConfig {
		t.Fatalf("skip report should flag the excluded ./.rune/plugins dir")
	}
...
func TestEmitTrustNoticeOneLineWhenSkipped(t *testing.T) {
	t.Run("both surfaces skipped yields one line", func(t *testing.T) {
		...
		if !bytes.Contains(buf.Bytes(), []byte("rune trust")) {
			t.Fatalf("notice should point at 'rune trust', got %q", buf.String())
		}
	})
