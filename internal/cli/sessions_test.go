...
func TestRunSessionsListsLineageAndTree(t *testing.T) {
	...
	if !strings.Contains(output, "Rune sessions") || !strings.Contains(output, root.SessionID) || !strings.Contains(output, child.SessionID) || !strings.Contains(output, "code-review") || !strings.Contains(output, "tag=specialist") || !strings.Contains(output, "depth=1") {
...
func TestRunSessionsValidatesArgs(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "Rune session not found: missing") {
