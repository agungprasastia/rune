...
func TestRunProvidersCheckMissingKeyPrintsNextAction(t *testing.T) {
	...
	if !strings.Contains(output, "next: set GROQ_API_KEY and rerun rune providers check groq") {
...
func TestRunProvidersCheckConnectivitySkippedPrintsNextAction(t *testing.T) {
	...
	if !strings.Contains(output, "next: run rune providers check work --connectivity") {
...
func TestRunProvidersCheckConnectivityFailurePrintsGuidance(t *testing.T) {
	...
	if !strings.Contains(output, "next: verify the API key, base URL, and model, then rerun rune providers check local --connectivity") {
...
func TestRunProvidersCheckConnectivityOKPrintsExecNextAction(t *testing.T) {
	...
	output := stdout.String()
	if !strings.Contains(output, `next: run rune exec "hello" --model gpt-4.1`) {
