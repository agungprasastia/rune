...
func TestWriteExecProviderErrorAppendsHint(t *testing.T) {
	t.Run("provider auth error gets a hint", func(t *testing.T) {
		...
		if !strings.Contains(out, "rune setup") || !strings.Contains(out, "rune auth openrouter") {
	...
	t.Run("non-provider error gets no hint", func(t *testing.T) {
		...
		out := stderr.String()
		// Exactly one "[rune]" line — no spurious hint attached to a local error.
		if n := strings.Count(out, "[rune]"); n != 1 {
			t.Fatalf("expected exactly one [rune] line for a non-provider error, got %d:\n%s", n, out)
		}
	})
	...
	t.Run("json mode never appends a hint line", func(t *testing.T) {
		...
		if strings.Contains(stdout.String(), "rune setup") {
