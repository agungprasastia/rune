...
// runTrust implements `rune trust`, letting the user opt a workspace into running
...
//
//	rune trust                trust the current working directory
//	rune trust list           print the trusted roots, one per line
//	rune trust remove [path]  untrust the current directory, or a named path
func runTrust(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	case "-h", "--help", "help":
		...
		return exitSuccess
	default:
		if _, err := fmt.Fprintf(stderr, "rune trust: unknown subcommand %q\n\n", args[0]); err != nil {
			return exitCrash
		}
...
func trustRemove(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	case 1:
		target = args[0]
	default:
		if _, err := fmt.Fprintln(stderr, "usage: rune trust remove [path]"); err != nil {
			return exitCrash
		}
		return exitUsage
	}
...
func writeTrustUsage(w io.Writer) {
  ...
  rune trust                Trust the current working directory
  rune trust list           List trusted workspace roots
  rune trust remove [path]  Untrust the current directory, or a named path

Trust lets Rune run a workspace's project-scoped hooks, plugins, and MCP
servers (./.rune/hooks.json, ./.rune/plugins/, project MCP config). Trust is
exact per directory.
`)
}
