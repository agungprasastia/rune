...
func TestWebhookSinkPostsExpectedJSON(t *testing.T) {
	...
	sink.Emit(Completion, "Rune: ready")
	...
	case r := <-got:
		...
		if r.body.Message != "Rune: ready" {
			t.Fatalf("payload message = %q, want %q", r.body.Message, "Rune: ready")
		}
...
func TestWebhookSinkIncludesSummaryAndLinks(t *testing.T) {
	...
	sink.Emit(AwaitingInput, "Rune: needs input")
