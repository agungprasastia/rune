...
func TestNotifyWritesAndRespectsPolicy(t *testing.T) {
	...
	n.Notify(Completion, "Rune: ready")
	if buf.String() != "\x07\x1b]9;Rune: ready\x07" {
...
func TestDefaultMessage(t *testing.T) {
	if DefaultMessage(Completion) != "Rune: ready" {
		t.Fatal("completion message")
	}
	if DefaultMessage(AwaitingInput) != "Rune: needs input" {
