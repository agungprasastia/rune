...
func TestParseManifestNormalizesExtensionsAndPaths(t *testing.T) {
	...
	pluginDir := filepath.Join(root, "rune-demo")
	...
	if plugin.ID != "rune.demo" || plugin.Name != "Rune Demo" || !plugin.Enabled {
...
func TestParseManifestWithoutOptionalMetadataLeavesZeroValues(t *testing.T) {
	root := filepath.Join(t.TempDir(), "plugins")
	pluginDir := filepath.Join(root, "rune-bare")
	manifestPath := filepath.Join(pluginDir, "plugin.json")
...
func TestLoadPluginsDiscoversDiagnosticsAndProjectPrecedence(t *testing.T) {
	...
	if got := []string{result.Plugins[0].ID + ":" + result.Plugins[0].Name, result.Plugins[1].ID + ":" + result.Plugins[1].Name}; got[0] != "rune.demo:Project Demo" || got[1] != "rune.docs:Docs" {
		t.Fatalf("unexpected plugins: %#v", result.Plugins)
	}
	if result.Plugins[0].Enabled {
		t.Fatalf("project plugin should remain disabled")
	}
	if !hasPluginDiagnostic(result.Diagnostics, DiagnosticDuplicate, "rune.demo") {
...
func TestLoadDiscoversProjectPluginWhenNotExcluded(t *testing.T) {
	dir := t.TempDir()
	writePluginManifest(t, filepath.Join(dir, ".rune", "plugins", "demo"), map[string]any{
		"schemaVersion": 1,
		"id":            "rune.project",
		"name":          "Project Plugin",
		"version":       "0.1.0",
	...
	if !hasPlugin(result.Plugins, "rune.project") {
...
func TestLoadKeepsUserPluginWhenProjectExcluded(t *testing.T) {
	dir := t.TempDir()
	// User plugin lives under the resolved XDG config home; project plugin under ./.rune.
	writePluginManifest(t, filepath.Join(dir, "xdg", "rune", "plugins", "user"), map[string]any{
		"schemaVersion": 1,
		"id":            "rune.user",
		"name":          "User Plugin",
		"version":       "0.1.0",
	...
	if !hasPlugin(result.Plugins, "rune.user") {
...
func TestResolveRootsUsesConfigHomeAndProjectRoot(t *testing.T) {
	...
	if roots[0].Path != filepath.Join(dir, "xdg", "rune", "plugins") {
		t.Fatalf("user root = %q", roots[0].Path)
	}
	if roots[1].Path != filepath.Join(dir, ".rune", "plugins") {
