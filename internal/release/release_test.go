...
func TestCopyPackageFilesStagesLinuxSandboxHelper(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(staging, "rune-linux-sandbox")); err != nil {
		t.Fatalf("staged helper missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "rune-seccomp")); err != nil {
...
func TestCopyPackageFilesStagesWindowsSandboxHelpers(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(staging, "rune-windows-command-runner.exe")); err != nil {
		t.Fatalf("staged runner missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "rune-windows-sandbox-setup.exe")); err != nil {
