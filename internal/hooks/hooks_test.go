...
func TestResolvePathsUsesXDGLocations(t *testing.T) {
	...
	if paths.UserConfigPath != filepath.Join(dir, "config", "rune", "hooks.json") {
		t.Fatalf("user path = %q", paths.UserConfigPath)
	}
	if paths.ProjectConfigPath != filepath.Join(dir, ".rune", "hooks.json") {
		t.Fatalf("project path = %q", paths.ProjectConfigPath)
	}
	if paths.AuditPath != filepath.Join(dir, "data", "rune", "hooks", "audit.jsonl") {
...
func TestLoadConfigLayersProjectOverridesAndDiagnostics(t *testing.T) {
	...
	if got := []string{result.Config.Hooks[0].ID, result.Config.Hooks[1].ID}; !reflect.DeepEqual(got, []string{"rune.audit", "rune.format"}) {
	...
	if !hasHookDiagnostic(result.Diagnostics, DiagnosticDuplicate, "rune.format", "") {
...
func TestLoadConfigExcludeProjectSkipsProjectLayer(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "include project (default)", excludeProject: false, wantIDs: []string{"rune.project", "rune.user"}},
		{name: "exclude project", excludeProject: true, wantIDs: []string{"rune.user"}},
	} {
...
func TestConfigStorePersistsUpdates(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Upsert with rune-value Enabled returned error: %v", err)
	}
	if !upserted.Enabled {
		t.Fatalf("Upsert should default rune-value Enabled to true; use SetEnabled to disable: %#v", upserted)
	}
	changed, err := store.SetEnabled("rune.preflight", false)
	if err != nil || !changed {
	...
	if findHook(config.Hooks, "rune.preflight") != nil {
		t.Fatalf("expected rune.preflight to be removed, got %#v", config.Hooks)
	}
}
...
func TestConfigStoreListReadsOnlyStorePath(t *testing.T) {
	...
	if got := hookIDs(config.Hooks); !reflect.DeepEqual(got, []string{"rune.real"}) {
...
func TestSelectMatchesEnabledHooksByEventAndWildcard(t *testing.T) {
	config := Config{Enabled: true, Hooks: []Definition{
		{ID: "rune.reads", Event: EventBeforeTool, Matcher: "read_*", Command: "node", Enabled: true},
		{ID: "rune.shell", Event: EventBeforeTool, Matcher: "bash", Command: "node", Enabled: false},
		{ID: "rune.done", Event: EventSessionEnd, Command: "node", Enabled: true},
		{ID: "rune.shell-edit", Event: EventBeforeTool, Matcher: "shell_*_edit", Command: "node", Enabled: true},
	}}

	if got := hookIDs(Select(config, SelectInput{Event: EventBeforeTool, ToolName: "read_file"})); !reflect.DeepEqual(got, []string{"rune.reads"}) {
		t.Fatalf("read selection = %#v", got)
	}
	if got := hookIDs(Select(config, SelectInput{Event: EventBeforeTool, ToolName: "shell_safe_edit"})); !reflect.DeepEqual(got, []string{"rune.shell-edit"}) {
...
func TestSpecialistHookEventsLoadAndSelect(t *testing.T) {
	...
	writeHookJSON(t, projectConfigPath, map[string]any{
		"hooks": []any{
			map[string]any{
				"id":      "rune.specialist-start",
				"event":   "specialistStart",
				"command": "node",
			},
			map[string]any{
				"id":      "rune.specialist-stop",
	...
	if got := hookIDs(result.Config.Hooks); !reflect.DeepEqual(got, []string{"rune.specialist-start", "rune.specialist-stop"}) {
		t.Fatalf("hook ids = %#v", got)
	}
	if got := hookIDs(Select(result.Config, SelectInput{Event: EventSpecialistStart})); !reflect.DeepEqual(got, []string{"rune.specialist-start"}) {
		t.Fatalf("specialistStart selection = %#v", got)
	}
	if got := hookIDs(Select(result.Config, SelectInput{Event: EventSpecialistStop})); !reflect.DeepEqual(got, []string{"rune.specialist-stop"}) {
...
func TestAuditStoreAppendsAndSkipsMalformedLines(t *testing.T) {
	...
	if err := os.WriteFile(auditPath, []byte(strings.Join([]string{
		`{"sequence":1,"createdAt":"2026-06-04T00:00:00Z","type":"hook_execution_started","hookId":"rune.seed","event":"sessionStart"}`,
		"{not-json",
		`{"sequence":2,"createdAt":"2026-06-04T00:00:01Z","type":"hook_execution_completed","hookId":"rune.seed","event":"sessionStart","status":"completed"}`,
		"",
	}, "\n")), 0o600); err != nil {
		t.Fatal(err)
	}
