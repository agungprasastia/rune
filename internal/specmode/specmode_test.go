...
func TestSaveDraftWritesPlainMarkdownWithCollisionSuffix(t *testing.T) {
	...
	if first.RelativePath != ".rune/specs/2026-06-08-oauth-redirect.md" {
...
func TestSaveDraftContainsAdversarialTitles(t *testing.T) {
	...
	for _, title := range []string{
		...
		t.Run(title, func(t *testing.T) {
			...
			if filepath.IsAbs(saved.RelativePath) || !strings.HasPrefix(saved.RelativePath, ".rune/specs/") {
...
func TestSubmitToolSavesSpecAndReturnsReviewControl(t *testing.T) {
	...
	if len(result.ChangedFiles) != 1 || result.ChangedFiles[0] != ".rune/specs/2026-06-08-implementation-plan.md" {
...
func TestLoadSpecFileRejectsPathsOutsideSpecDirectory(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected LoadSpecFile to reject a path outside .rune/specs")
	}
}

func TestLoadSpecFileRejectsNonRegularFiles(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, filepath.FromSlash(".rune/specs/not-a-file.md"))
...
func TestImplementationPromptIncludesReviewContext(t *testing.T) {
	prompt := ImplementationPrompt("# Goal\n\nShip it.", "/repo/.rune/specs/plan.md", "rune_1", "Keep tests focused.")

	for _, want := range []string{
		"Implement the following approved spec:",
		"User note: Keep tests focused.",
		"# Goal\n\nShip it.",
		"Spec file: /repo/.rune/specs/plan.md",
		"Planning session: rune_1",
	} {
