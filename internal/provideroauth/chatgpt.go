...
type ChatGPTOptions struct {
	...
	// RUNE_OAUTH_ALLOW_PRESETS must be set in the effective env (or a preset-supplied
	// env block) for the baked-in client_id to be used; otherwise the caller is
	// expected to have configured RUNE_OAUTH_CHATGPT_* explicitly.
...
func ChatGPTLogin(ctx context.Context, opts ChatGPTOptions) (oauth.Token, error) {
	...
	// makes this work without RUNE_OAUTH_CHATGPT_CLIENT_ID: the resolver applies
	// it whenever RUNE_OAUTH_ALLOW_PRESETS is set, the same path every other
	...
	if err != nil {
		return oauth.Token{}, fmt.Errorf("provideroauth: start loopback listener on port %d (close any other Rune or Codex login already running and retry): %w", chatgptCallbackPort, err)
	}
