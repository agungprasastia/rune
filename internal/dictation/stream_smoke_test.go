...
//
//	RUNE_STT_STREAM_SERVER=/path/to/sherpa-onnx-online-websocket-server \
//	RUNE_STT_STREAM_MODEL=/path/to/streaming-model-dir \
//	RUNE_STT_STREAM_WAV=/path/to/16k-mono.wav \
//	go test ./internal/dictation/ -run RealServer -v -timeout 5m
func TestLocalStreamingRealServer(t *testing.T) {
	server := os.Getenv("RUNE_STT_STREAM_SERVER")
	model := os.Getenv("RUNE_STT_STREAM_MODEL")
	wav := os.Getenv("RUNE_STT_STREAM_WAV")
	if server == "" || model == "" || wav == "" {
		t.Skip("set RUNE_STT_STREAM_SERVER, RUNE_STT_STREAM_MODEL, RUNE_STT_STREAM_WAV to run the streaming smoke test")
	}
