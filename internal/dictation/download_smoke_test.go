...
// It downloads ~130MB, so it is opt-in via RUNE_STT_DOWNLOAD_TEST=1.
//
//	RUNE_STT_DOWNLOAD_TEST=1 go test ./internal/dictation/ -run RealDownload -v -timeout 10m
func TestEnsureLocalEngineRealDownload(t *testing.T) {
	if os.Getenv("RUNE_STT_DOWNLOAD_TEST") == "" {
		t.Skip("set RUNE_STT_DOWNLOAD_TEST=1 to run the real ~130MB download smoke test")
	}
	dest := t.TempDir()
