...
func TestResolvePermissionPathHonorsOverrideAndConfigHome(t *testing.T) {
	dir := t.TempDir()
	override, err := ResolvePermissionPath(map[string]string{"RUNE_MCP_PERMISSIONS_PATH": filepath.Join(dir, "custom.json")})
	if err != nil {
	...
	if resolved != filepath.Join(dir, "xdg", "rune", "mcp-permissions.json") {
