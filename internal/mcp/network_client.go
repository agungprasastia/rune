...
func (transport *oauthRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if refreshErr != nil {
		return nil, fmt.Errorf("MCP OAuth token refresh failed for %s: re-run `rune mcp oauth login %s`: %w", transport.serverName, transport.serverName, refreshErr)
	}
...
func (source *storeTokenSource) AccessToken(ctx context.Context) (string, error) {
	...
	if !ok || strings.TrimSpace(token.AccessToken) == "" {
		return "", fmt.Errorf("no stored OAuth token for MCP server %s: run `rune mcp oauth login %s`", source.server.Name, source.server.Name)
	}
