...
func TestServeListsAndCallsRegistryTools(t *testing.T) {
	...
	writeServerTestMessage(t, &input, rpcMessage{
		...
		Params: mustRaw(map[string]any{
			"name":      "lookup",
			"arguments": map[string]any{"query": "rune"},
		}),
	})

	var output bytes.Buffer
	if err := Serve(context.Background(), &input, &output, registry, ServeOptions{Name: "rune-test", Version: "1.2.3"}); err != nil {
	...
	if initResult.ProtocolVersion != defaultProtocolVersion || initResult.ServerInfo.Name != "rune-test" || initResult.ServerInfo.Version != "1.2.3" {
	...
	if got := TextContent(callResult.Content); got != "lookup: rune" {
		t.Fatalf("call result text = %q, want lookup: rune", got)
	}
}
