...
func run(args []string, stdout io.Writer, stderr io.Writer) int {
	if len(args) == 0 {
		_, _ = fmt.Fprintln(stderr, "rune-release command required. Use `rune-release --help`.")
		return 2
	}
	switch args[0] {
	case "-h", "--help", "help":
	...
	default:
		_, _ = fmt.Fprintf(stderr, "unknown rune-release command %q\n", args[0])
		return 2
	}
}
