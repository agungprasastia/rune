...
This document explains Rune's architecture at a high level: the layers, module
boundaries, data flow, and runtime concepts behind the interactive TUI and the
headless `rune exec` mode.
...
Rune is a Go CLI application with two main user-facing surfaces:

- **Interactive TUI** (`rune`): a Bubble Tea application that renders chat,
  tool cards, permission prompts, model/provider pickers, and session controls.
- **Headless exec** (`rune exec ...`): a scriptable command runner that uses the
...
```mermaid
flowchart TD
    User[User / automation] --> CLI[cmd/rune -> internal/cli]
    ...
    Agent --> Provider[Provider runtime<br/>internal/runeruntime.Provider]
...
The easiest way to understand Rune is not as a set of independent packages, but
...
| Package | Role |
| --- | --- |
| `cmd/rune` | Tiny binary entrypoint; calls `internal/cli.Run`. |
...
| `internal/runeruntime` | Provider-neutral message, tool-call, stream, usage, and image types. Providers implement `Provider.StreamCompletion`. |
...
| `internal/streamjson` | Machine-readable protocol for `rune exec --input-format/--output-format stream-json`. |

## Startup Flow

### Interactive TUI (`rune`)

```mermaid
sequenceDiagram
    participant Main as cmd/rune
...
`rune exec` uses the same `agent.Run` loop as the TUI, but without interactive
...
3. **Call provider** — sends a provider-neutral `runeruntime.CompletionRequest`.
...
The model never directly mutates the workspace. It asks for tool calls; Rune
...
it. The model proposes an action, Rune decides whether that action is allowed to
...
1. **Proposal** — the provider streams assistant content and may emit one or more
   tool calls. Rune can show the partial tool arguments in the TUI while they are
...
4. **Normalize result** — Rune turns the outcome into an agent `ToolResult`: a
...
TUI. Rune controls the loop mechanics: it keeps appending messages, enforces
...
The `Maybe compact context` step in the agent-loop diagram is Rune's context-window
...
- Rune estimates request size using message text, images, tool-call arguments, and
...
2. **Reactive compaction** runs after a provider returns a context-limit style
   error. Rune compacts once and retries that same turn; if the provider still
...
compaction, Rune keeps the original messages and tries to continue. It does not
...
A common source of confusion is that Rune has two event loops running at once:
...
- **Stream-JSON** is another surface over the same callbacks: `rune exec`
...
Providers are hidden behind this interface in `internal/runeruntime`:
...
Rune has two related but separate checks:
...
The user-facing TUI and `rune exec` mode both route permission requests through
...
Sessions live locally under Rune's data directory. A session directory contains:
...
remote service. Rune stores the local event history; providers only receive the
messages needed for the current completion request.

## Stream-JSON Protocol

`rune exec --output-format stream-json` emits newline-delimited JSON events for
...
Rune's extension surfaces are loaded before the agent loop starts:
...
flowchart TD
    ...
    Prompt --> Guidelines[AGENTS.md / RUNE.md]
...
- **Plugins** can add tools, hooks, and skill roots. Bootstrap always registers a
  multi-root skill tool: primary Rune skills dir, optional `~/.agents/skills`,
...
The same request in `rune exec` follows the same provider/tool/sandbox path, but
streaming events are written to stdout/stderr instead of rendered in Bubble Tea.

## Mental Model

When you work on Rune, start by locating the change on the four-layer spine from
