...
Rune agent evals are maintainer fixtures for checking coding-agent behavior
...
`rune eval` defaults to `validate` mode. In validate mode, the command performs
...
```bash
go run ./cmd/rune eval --suite internal/agenteval/testdata/sample_suite.json
```

Use JSON output when another local tool needs the validation summary:

```bash
go run ./cmd/rune eval --suite internal/agenteval/testdata/sample_suite.json --json
```

### Run Mode

`rune eval run` scores one already-mutated Git workspace. It does not copy
...
go run ./cmd/rune eval run \
  ...
  --task document-stream-json-verify-events \
  --workspace /tmp/rune-eval-workspace \
  --report-dir /tmp/rune-eval-report \
  --json
```

### Bench Mode

`rune eval bench` runs the full benchmark harness for one task or a suite. Bench
...
```bash
go run ./cmd/rune eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --task document-stream-json-verify-events \
  --work-root /tmp/rune-evals \
  --agent-command rune exec --cwd {workspace} {prompt}
```
...
go run ./cmd/rune eval bench \
  ...
  --work-root /tmp/rune-evals \
  --model gpt-5 \
  --agent-command rune exec --model {model} --cwd {workspace} {prompt}
```
...
go run ./cmd/rune eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --work-root /tmp/rune-evals \
  --agent-command rune-agent-wrapper --task {task_id} --workspace {workspace} --prompt {prompt}
```
...
go run ./cmd/rune eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --task add-native-rune-argv-helper \
  --work-root /tmp/rune-evals \
  --keep-workspaces \
  --report-dir /tmp/rune-eval-report \
  --json \
...
Scored reports use contract `rune.agenteval.report.v1`.
