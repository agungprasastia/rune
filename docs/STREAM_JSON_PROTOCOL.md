# Rune Stream-JSON Protocol

Rune stream-json is a line-delimited protocol for headless clients such as
...
`rune exec --input-format stream-json` accepts these JSONL events from stdin or
`--file`.
...
Rune combines accepted input event content in order, separated by blank lines.
...
`rune exec --output-format stream-json` emits schema-versioned JSONL events.

```json
{ "schemaVersion": 2, "type": "run_start", "runId": "run_20260603_abc123", "sessionId": "rune_20260603100000_abc123", "cwd": "/repo", "provider": "openai", "model": "gpt-4.1", "apiModel": "gpt-4.1" }
...
is not pre-approved, Rune may emit `permission_request` followed by a denied
